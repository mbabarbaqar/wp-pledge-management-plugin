 ...
 *
 * Override this template by copying it to yourtheme/hspg/donation-form/donation-amount-list.php
 *
 * @author  Studio 164a
			...
			<li class="donation-amount suggested-donation-amount <?php echo strlen( $checked ) ? 'selected' : ''; ?>">
				<label for="<?php echo $field_id; ?>">
					<input
						...
						value="<?php echo esc_attr( hs_get_currency_helper()->sanitize_database_amount( $suggestion['amount'] ) ); ?>" <?php echo $checked; ?>
					/>
					<?php
						printf(
							'<span class="amount">%s</span> <span class="description">%s</span>',
							hs_format_money( $suggestion['amount'], false, true ),
			...
			<li class="donation-amount custom-donation-amount">
				<span class="custom-donation-amount-wrapper">
					<label for="form-<?php echo esc_attr( $form_id ); ?>-field-custom-amount">
						...
						/><span class="description"><?php echo apply_filters( 'hs_donation_amount_form_custom_amount_text', __( 'Custom amount', 'hspg' ) ); ?></span>
					</label>
					<input
						type="text"
...
<?php elseif ( $custom ) : ?>
	<div id="custom-donation-amount-field" class="hs-form-field hs-custom-donation-field-alone">
		<input
			type="text"
			class="custom-donation-input"
			name="custom_donation_amount"
			placeholder="<?php esc_attr_e( 'Enter donation amount', 'hspg' ); ?>"
