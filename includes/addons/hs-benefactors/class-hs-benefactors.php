<?php
/**
 * Main class for setting up the Hspg Benefactors Addon, which is programatically activated by child themes.
 *
 * @package   Hspg/Classes/Hs_Benefactors
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Benefactors' ) ) :
	 ...
	 * @since   1.0.0
	 */
	class Hs_Benefactors implements Hs_Addon_Interface {

		/**
		 * The single instance of this class.
		 *
		 * @var     Hs_Benefactors|null
		 */
		...
		private function __construct() {
			require_once( 'class-hs-benefactor.php' );
			require_once( 'class-hs-benefactors-db.php' );
			require_once( 'hs-benefactors-hooks.php' );
		}
		...
		public static function load() {
			do_action( 'hs_benefactors_addon_loaded', Hs_Benefactors::get_instance() );
		}
		...
		public function register_script() {
			...
			if ( 'campaign' == $screen->id ) {
				...
				wp_register_script( 'hs-benefactors-js', hspg()->get_path( 'assets', false ) . 'js/hs-admin-benefactors' . $suffix . '.js', array( 'hs-admin' ), hspg()->get_version(), false );
				wp_enqueue_script( 'hs-benefactors-js' );
			}
		}
		...
		public function register_table( $tables ) {
			$tables['benefactors'] = 'Hs_Benefactors_DB';
			return $tables;
		}
		 ...
		 * @since   1.0.0
		 *
		 * @param   Hs_Benefactor $benefactor Benefactor object.
		...
		public function benefactor_meta_box( $benefactor, $extension ) {
			hs_admin_view( 'metaboxes/campaign-benefactors/summary', array(
		...
		public function save_benefactors( WP_Post $post ) {
			...
			foreach ( $benefactors as $campaign_benefactor_id => $data ) {
				...
				if ( isset( $data['date_created'] ) && strlen( $data['date_created'] ) ) {
					if ( $sanitize_date ) {
						$data['date_created'] = hs_sanitize_date( $data['date_created'], 'Y-m-d 00:00:00' );
					} else {
				...
				if ( isset( $data['date_deactivated'] ) && strlen( $data['date_deactivated'] ) ) {
					if ( $sanitize_date ) {
						$date_deactivated = hs_sanitize_date( $data['date_deactivated'], 'Y-m-d 00:00:00' );
					} else {
				...
				if ( 0 == $campaign_benefactor_id ) {
					hs_get_table( 'benefactors' )->insert( $data );
				} else {
					hs_get_table( 'benefactors' )->update( $campaign_benefactor_id, $data );
				}
			}//end foreach
		}
		...
		public function delete_benefactor() {
			...
			check_ajax_referer( 'hs-deactivate-benefactor', 'nonce' );

			$benefactor_id = isset( $_POST['benefactor_id'] ) ? $_POST['benefactor_id'] : 0;

			if ( ! $benefactor_id ) {
				$return = array( 'error' => __( 'No benefactor ID provided.', 'hspg' ) );
			} else {
				$deleted = hs_get_table( 'benefactors' )->delete( $benefactor_id );
		...
		/**
		 * Called when Hspg is uninstalled and data removal is set to true.
		 *
		 * @since   1.0.0
		 *
		 * @return  void
		...
		public function uninstall() {
			if ( 'hs_uninstall' != current_filter() ) {
				return;
			}
