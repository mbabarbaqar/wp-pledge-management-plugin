<?php
/**
 * Hspg Benefactors DB class.
 *
 * @package     Hspg/Classes/Hs_Benefactors_DB
 * @version  	1.0.0
 * @author 		Eric Daams
...
if ( ! class_exists( 'Hs_Benefactors_DB' ) ) :
	 ...
	 * @since   1.0.0
	 */
	class Hs_Benefactors_DB extends Hs_DB {
		...
		public function __construct() {
			...
			$this->table_name = $wpdb->prefix . 'hs_benefactors';
		}
		...
		public function insert( $data, $type = 'campaign_benefactor' ) {
			...
			$data = apply_filters( 'hs_benefactor_data', $data );
			...
			if ( ! isset( $data['benefactor'] ) || ! is_array( $data['benefactor'] ) || empty( $data['benefactor'] ) ) {
				...
				hs_get_deprecated()->doing_it_wrong(
					__METHOD__,
					__( 'Campaign benefactors cannot be created without benefactor details.', 'hspg' ),
					'1.0.0'
				);
			...
			if ( empty( $data['contribution_amount'] ) || ! is_numeric( $data['contribution_amount'] ) ) {
				...
				hs_get_deprecated()->doing_it_wrong(
					__METHOD__,
					__( 'Campaign benefactors cannot be created without a contribution amount.', 'hspg' ),
					'1.0.0'
				);
			...
			do_action( 'hs_benefactor_added', $campaign_benefactor_id, $benefactor_details, $data );
		...
		public function update( $row_id, $data = array(), $where = '' ) {
			...
			if ( isset( $data['benefactor'] ) ) {
				...
				/* Allow plugins to hook into this event */
				do_action( 'hs_benefactor_updated', $row_id, $benefactor_details, $where );
			}
		...
		public function delete( $row_id = 0 ) {
			...
			/* Allow plugins to hook into this event */
			do_action( 'hs_benefactor_deleted', $row_id );
		...
		public function get_campaign_benefactors_by_extension( $campaign_id, $extension ) {
			return apply_filters( 'hs_get_campaign_benefactors', false, $campaign_id, $extension );
		}
	}
