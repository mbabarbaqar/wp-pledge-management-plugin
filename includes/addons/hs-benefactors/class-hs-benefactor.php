<?php
/**
 * The model for Benefactor relationships between Hspg campaigns and products in 3rd party extensions (EDD, WooCommerce, etc).
 *
 * @package		Hspg/Classes/Hs_Benefactor
 * @version 	1.0.0
 * @author 		Eric Daams
	...
	abstract class Hs_Benefactor {
		...
		public function __construct( $benefactor ) {
			if ( ! is_object( $benefactor ) ) {
				$this->benefactor = hs_get_table( 'benefactors' )->get( $benefactor );
			}
		}

		/**
		 * Return an object of type Hs_Benefactor, given a benefactor record and an extension.
		 *
		 * @since   1.0.0
		 *
		 * @param 	Object $benefactor Benefactor object.
		 * @param 	string $extension  Extension generating the benefactor object.
		 * @return  Hs_Benefactor
		 */
		public static function get_object( $benefactor, $extension ) {
			$class = apply_filters( 'hs_benefactor_class_' . $extension, false );

			if ( ! class_exists( $class ) ) {
				hs_get_deprecated()->doing_it_wrong(
					__METHOD__,
					__( 'Benefactor class does not exist for given extension.', 'hspg' ),
					'1.0.0'
				);
			}
		...
		public function __toString() {
			return apply_filters( 'hs_benefactor_summary', $this->get_contribution_description(), $this );
		}
		...
		public function get_contribution_amount() {
			if ( $this->benefactor->contribution_amount_is_percentage ) {
				$amount = apply_filters( 'hs_benefactor_contribution_amount_percentage', $this->benefactor->contribution_amount . '%', $this->benefactor->contribution_amount, $this );
			} else {
				$amount = apply_filters( 'hs_benefactor_contribution_amount_fixed', hs_format_money( $this->benefactor->contribution_amount ), $this->benefactor->contribution_amount, $this );
			}
