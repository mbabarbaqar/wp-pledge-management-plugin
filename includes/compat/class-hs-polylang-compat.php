 ...
 *
 * @package   Hspg/Classes/Hs_Polylang_Compat
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Polylang_Compat' ) ) :
	...
	class Hs_Polylang_Compat {
		...
		public function __construct() {
			...
			add_filter( 'hs_option_terms_conditions_page', array( $this, 'get_polylang_page_id' ) );
			add_filter( 'hs_option_privacy_policy_page', array( $this, 'get_polylang_page_id' ) );
			 ...
			 * @since 1.6.42
			 *
			 * @param Hs_Polylang_Compat $helper Polylang compatibility class instance.
			 */
			do_action( 'hs_polylang_compat', $this );
		}

		/**
		 * When Polylang picks up the language from the content, it translates
		 * content late, on the 'wp' hook. This is after Hspg's donation
		...
		public function load_late_translations() {
			...
			$field_apis = [
				[
					'fields' => hspg()->donation_fields(),
					'forms'  => [ 'donation_form', 'admin_form' ],
				],
				[
					'fields' => hspg()->campaign_fields(),
		...
		public function get_user_dashboard_menus() {
			...
			foreach ( $locations as $location => $assigned_menu ) {
				if ( 'hs-dashboard' === substr( $location, 0, 20 ) ) {
