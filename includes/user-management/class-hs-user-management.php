...
if ( ! class_exists( 'Hs_User_Management' ) ) :

	/**
	 * Hs_User_Management class
	 *
	 * @since 1.4.0
	 */
	class Hs_User_Management {

		/**
		 * The class instance.
		 *
		 * @var   Hs_User_Management
		 *
		 * @since 1.4.0
		 */
		...
		public function maybe_redirect_to_password_reset() {
			...
			wp_safe_redirect( esc_url_raw( hs_get_permalink( 'reset_password_page' ) ) );
		 ...
		 *
		 * If so, and hs_disable_wp_login is set, redirect them to the custom reset password page
		 *
		 * @since  1.4.0
		 *
		 * @return void
		...
		public function maybe_redirect_to_custom_password_reset_page() {
			if ( ! apply_filters( 'hs_disable_wp_login', false ) ) {
				return;
			}

			if ( 'wp' == hs_get_option( 'login_page', 'wp' ) ) {
				return;
			}

			$redirect_url = hs_get_permalink( 'reset_password_page' );
		...
		/**
		 * Check if a failed user login attempt originated from Hspg login form.
		 *
		 * If so redirect user to Hspg login page.
		 *
		 * @since  1.4.0
		 *
		...
		public function maybe_redirect_at_authenticate( $user_or_error, $username ) {
			...
			if ( ! isset( $_POST['hspg'] ) || ! $_POST['hspg'] ) {
				return $user_or_error;
			}
			...
			foreach ( $user_or_error->errors as $code => $error ) {
				...
				switch ( $code ) {
					...
					case 'invalid_email':
						$error = __( '<strong>ERROR</strong>: Invalid email address.', 'hspg' ) .
							' <a href="' . esc_url( hs_get_permalink( 'forgot_password_page' ) ) . '">' .
				...
				hs_get_notices()->add_error( $error );
			}

			hs_get_session()->add_notices();

			$redirect_url = hs_get_permalink( 'login_page' );
		 ...
		 *
		 * If so, and hs_disable_wp_login is set, redirect them to the custom forgot password page
		 *
		 * @since  1.4.0
		 *
		 * @return void
		...
		public function maybe_redirect_to_custom_lostpassword() {
			...
			wp_safe_redirect( esc_url_raw( hs_get_permalink( 'forgot_password_page' ) ) );
		...
		public function maybe_remove_admin_bar() {
			 ...
			 *
			 * add_filter( 'hs_disable_admin_bar', '__return_true' );
			 */
			if ( ! apply_filters( 'hs_disable_admin_bar', true ) ) {
				return;
			}
		...
		public function maybe_redirect_away_from_admin() {
			 ...
			 * you can use this one-liner:
			 *
			 * add_filter( 'hs_disable_admin_access', '__return_true' );
			 */
			if ( ! apply_filters( 'hs_disable_admin_access', true ) ) {
				return;
			}
			 ...
			 */
			$redirect_url = apply_filters( 'hs_admin_redirect_url', false );

			if ( ! $redirect_url ) {
				$redirect_url = hs_get_permalink( 'profile_page' );
		...
		/**
		 * Redirect the user to the Hspg login page.
		 *
		 * @since  1.4.0
		 *
		 * @return void
		 */
		public function maybe_redirect_to_hs_login() {
			if ( ! apply_filters( 'hs_disable_wp_login', false ) ) {
				return;
			}
			...
			wp_safe_redirect( esc_url_raw( hs_get_permalink( 'login_page' ) ) );
		...
		public function send_verification_email( $user = '', $redirect_url = '', $force_send = null ) {
			...
			/* Prepare the email. */
			$email = new Hs_Email_Email_Verification( array( 'user' => $user ) );
		...
		private function user_has_admin_access() {
			...
			$has_access = current_user_can( 'edit_posts' )
				|| current_user_can( 'manage_hs_settings' )
			 ...
			 */
			return apply_filters( 'hs_user_has_admin_access', $has_access );
		}
	}
