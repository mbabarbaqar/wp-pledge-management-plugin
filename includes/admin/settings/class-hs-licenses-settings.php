<?php
/**
 * Hspg Licenses Settings UI.
 *
 * @package     Hspg/Classes/Hs_Licenses_Settings
 * @version     1.0.0
 * @author     HCS
...
if ( ! class_exists( 'Hs_Licenses_Settings' ) ) :

	/**
	 * Hs_Licenses_Settings
	 *
	 * @final
	 * @since   1.0.0
	 */
	final class Hs_Licenses_Settings {

		/**
		 * The single instance of this class.
		 *
		 * @var     Hs_Licenses_Settings|null
		 */
		...
		public function maybe_add_licenses_tab( $tabs ) {
			...
			$products = hs_get_helper( 'licenses' )->get_products();
		...
		public function add_licenses_fields() {
			if ( ! hs_is_settings_view( 'licenses' ) ) {
				return array();
			}
			...
			foreach ( hs_get_helper( 'licenses' )->get_products() as $key => $product ) {
				$fields[ $key ] = array(
					'type'     => 'text',
					'render'   => false,
					'priority' => 6,
		...
		public function render_licenses_table() {
			hs_admin_view( 'settings/licenses' );
		}
		...
		public function add_license_recheck_button( $button ) {
			$licenses = array_filter( hs_get_helper( 'licenses' )->get_licenses(), 'is_array' );

			if ( empty( $licenses ) ) {
				return $button;
			}

			$html = '<input style="margin-left:8px;height:29px;" type="submit" class="button button-secondary" name="recheck" value="' . esc_attr__( 'Save & Re-check All Licenses', 'hspg' ) . '" /></p>';
		...
		public function save_license( $values, $new_values ) {
			...
			foreach ( $licenses as $product_key => $license ) {
				...
				$license_data = hs_get_helper( 'licenses' )->verify_license( $product_key, $license, $re_check );
