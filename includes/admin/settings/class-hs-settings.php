 ...
 * Hspg Settings UI.
 *
 * @package   Hspg/Classes/Hs_Settings
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Settings' ) ) :
	 ...
	 * @since 1.0.0
	 */
	final class Hs_Settings {

		/**
		 * The single instance of this class.
		 *
		 * @var  Hs_Settings|null
		 */
		...
		private function __construct() {
			do_action( 'hs_admin_settings_start', $this );
		}
		...
		public function get_sections() {
			...
			return apply_filters(
				'hs_settings_tabs',
				array(
					'general'  => __( 'General', 'hspg' ),
					'gateways' => __( 'Payment Gateways', 'hspg' ),
					'emails'   => __( 'Emails', 'hspg' ),
					'privacy'  => __( 'Privacy', 'hspg' ),
					'advanced' => __( 'Advanced', 'hspg' ),
				)
			);
		...
		public function maybe_add_extensions_tab( $tabs ) {
			 ...
			 */
			$settings = apply_filters( 'hs_settings_tab_fields_extensions', array() );

			/* Set the tab back to whatever it actually is */
			$_GET['tab'] = $actual_tab;

			if ( ! empty( $settings ) ) {
				$tabs = hs_add_settings_tab(
					$tabs,
					'extensions',
					__( 'Extensions', 'hspg' ),
					array(
		...
		public function register_settings() {
			if ( ! hs_is_settings_view() ) {
				return;
			}

			register_setting( 'hs_settings', 'hs_settings', array( $this, 'sanitize_settings' ) );
			...
			foreach ( $sections as $section_key => $section ) {
				$section_id = 'hs_settings_' . $section_key;
				...
				uasort( $section_fields, 'hs_priority_sort' );
		...
		public function sanitize_settings( $values ) {
			$old_values = get_option( 'hs_settings', array() );
			$new_values = array();
			 ...
			 */
			$values = apply_filters( 'hs_save_settings', $values, $new_values, $old_values );

			$this->add_update_message( __( 'Settings saved', 'hspg' ), 'success' );
		...
		public function render_field( $args ) {
			...
			hs_admin_view( 'settings/' . $field_type, $args );
		}
		...
		public function get_pages() {
			if ( ! isset( $this->pages ) ) {
				$this->pages = hs_get_pages_options();
			}
		...
		public function add_update_message( $message, $type = 'error', $dismissible = true ) {
			...
			hs_get_admin_notices()->add_notice( $message, $type, false, $dismissible );
		}
		...
		private function register_field( $field, $keys ) {
			$section_id = 'hs_settings_' . $keys[0];
			...
			add_settings_field(
				sprintf( 'hs_settings_%s', implode( '_', $keys ) ),
				$label,
				$callback,
				$section_id,
				$section_id,
		...
		private function get_field_classes( $field ) {
			$classes = array( 'hs-settings-field' );
			 ...
			 * @param array $field   The field definition.
			 */
			$classes = apply_filters( 'hs_settings_field_classes', $classes, $field );
		 ...
		 *
		 * @uses   hs_settings_fields
		 * @see    Hs_Settings::register_setting()
		 * @since  1.0.0
		 *
		 * @return array
		 */
		private function get_fields() {
			/**
			 * Use the hs_settings_tab_fields to include the fields for new tabs.
			...
			foreach ( $this->get_sections() as $section_key => $section ) {
				 ...
				 */
				$fields[ $section_key ] = apply_filters( 'hs_settings_tab_fields_' . $section_key, array() );
			}
			 ...
			 * @param array $fields Array of fields.
			 */
			return apply_filters( 'hs_settings_tab_fields', $fields );
		}
		...
		private function get_setting_submitted_value( $key, $field, $submitted, $section ) {
			 ...
			 * @param string $section   The section being saved.
			 */
			$value = apply_filters( 'hs_sanitize_value', $value, $field, $submitted, $key, $section );

			/**
			 * Sanitize the setting value.
			 *
			 * The filter hook is formatted like this: hs_sanitize_value_{$section}_{$key}.
			 *
			 * @since 1.5.0
			 *
			 * @param mixed $value     The current setting value.
			 ...
			 * @param array $submitted All submitted data.
			 */
			return apply_filters( 'hs_sanitize_value_' . $section . '_' . $key, $value, $field, $submitted );
		}
		...
		private function get_dynamic_groups() {
			if ( ! isset( $this->dynamic_groups ) ) {
				 ...
				 */
				$this->dynamic_groups = apply_filters( 'hs_dynamic_groups', array() );
			}
		...
		public function get_update_messages() {
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.4.13',
				'Hs_Admin_Notices::get_notices()'
			);

			return hs_get_admin_notices()->get_notices();
		}
	}
