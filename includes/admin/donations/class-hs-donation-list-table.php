 ...
 *
 * @package   Hspg/Classes/Hs_Donation_List_Table
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Donation_List_Table' ) ) :

	/**
	 * Hs_Donation_List_Table class.
	 *
	 * @final
	 * @since 1.5.0
	 */
	final class Hs_Donation_List_Table {

		/**
		 * The single instance of this class.
		 *
		 * @var Hs_Donation_List_Table|null
		 */
		...
		public function __construct() {
			do_action( 'hs_admin_donation_post_type_start', $this );
		}
		...
		public function dashboard_columns() {
			...
			return apply_filters(
				'hs_donation_dashboard_column_names',
				array(
					'cb'            => '<input type="checkbox"/>',
					'id'            => __( 'Donation', 'hspg' ),
					'amount'        => __( 'Amount Donated', 'hspg' ),
					'campaigns'     => __( 'Campaign(s)', 'hspg' ),
					'donation_date' => __( 'Date', 'hspg' ),
					'post_status'   => __( 'Status', 'hspg' ),
				)
			);
		}
		...
		public function dashboard_column_item( $column_name, $post_id ) {
			$donation = hs_get_donation( $post_id );

			switch ( $column_name ) {

				case 'id':
					$title = esc_attr__( 'View Donation Details', 'hspg' );
					$name  = $donation->get_donor()->get_name();

					if ( $name ) {
						$text = sprintf( _x( '#%d by %s', 'number symbol', 'hspg' ),
							$post_id,
							$name
						);
					} else {
						$text = sprintf( _x( '#%d', 'number symbol', 'hspg' ),
							$post_id
						);
					}
				...
				case 'amount':
					$display  = hs_format_money( $donation->get_total_donation_amount() );
					$display .= '<span class="meta">' . sprintf( _x( 'via %s', 'hspg' ), $donation->get_gateway_label() ). '</span>';
					break;
			 ...
			 * @param Hs_Donation $donation    The Hs_Donation object.
			 */
			echo apply_filters( 'hs_donation_column_display', $display, $column_name, $post_id, $donation );
		}
		...
		public function row_actions( $actions, $post ) {
			if ( Hspg::DONATION_POST_TYPE !== $post->post_type ) {
				return $actions;
			}
		...
		public function set_status_views( $views ) {
			...
			foreach ( hs_get_valid_donation_statuses() as $key => $label ) {
		...
		public function bulk_action_handler( $redirect_to, $action, $post_ids ) {
			...
			$donation_statuses = hs_get_valid_donation_statuses();

			$new_status    = str_replace( 'set-', '', $action ); // get the status name from action
			$report_action = 'bulk_' . Hspg::DONATION_POST_TYPE . '_status_update';
			...
			foreach ( $post_ids as $post_id ) {
				...
				do_action( 'hs_donations_table_do_bulk_action', $post_id, $new_status );
			}
		...
		public function get_bulk_actions() {
			...
			foreach ( hs_get_valid_donation_statuses() as $status_key => $label ) {
				$actions[ 'set-' . $status_key ] = sprintf( _x( 'Set to %s', 'set donation status to x', 'hspg' ), $label );
			}
			 ...
			 */
			return apply_filters( 'hs_donations_table_bulk_actions', $actions );
		}
		...
		public function bulk_admin_footer() {
			...
			if ( Hspg::DONATION_POST_TYPE == $post_type ) {
		...
		public function process_bulk_action() {
			...
			if ( ! isset( $_REQUEST['post_type'] ) || Hspg::DONATION_POST_TYPE !== $_REQUEST['post_type'] || ! isset( $_REQUEST['post'] ) ) {
				return;
			}
		...
		public function bulk_admin_notices() {
			...
			if ( 'edit.php' !== $pagenow || Hspg::DONATION_POST_TYPE !== $post_type ) {
				return;
			}
			...
			if ( ! empty( $_REQUEST[ $report_action ] ) ) {
				...
				$message = sprintf( _n( 'Donation status changed.', '%s donation statuses changed.', $number, 'hspg' ), number_format_i18n( $number ) );
		...
		public function post_messages( $messages ) {
			...
			$messages[ Hspg::DONATION_POST_TYPE ] = array(
				...
				1 => sprintf( __( 'Donation updated. <a href="%s">View Donation</a>', 'hspg' ), esc_url( get_permalink( $post_ID ) ) ),
				2 => __( 'Custom field updated.', 'hspg' ),
				3 => __( 'Custom field deleted.', 'hspg' ),
				4 => __( 'Donation updated.', 'hspg' ),
				5 => isset( $_GET['revision'] ) ? sprintf( __( 'Donation restored to revision from %s', 'hspg' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
				6 => sprintf( __( 'Donation published. <a href="%s">View Donation</a>', 'hspg' ), esc_url( get_permalink( $post_ID ) ) ),
				...
				11 => __( 'Donation updated and email sent.', 'hspg' ),
				12 => __( 'Email could not be sent.', 'hspg' ),
			);
		...
		public function bulk_messages( $bulk_messages, $bulk_counts ) {
			$bulk_messages[ Hspg::DONATION_POST_TYPE ] = array(
				'updated'   => _n( '%d donation updated.', '%d donations updated.', $bulk_counts['updated'], 'hspg' ),
				'locked'    => ( 1 == $bulk_counts['locked'] ) ? __( '1 donation not updated, somebody is editing it.' ) :
								   _n( '%s donation not updated, somebody is editing it.', '%s donations not updated, somebody is editing them.', $bulk_counts['locked'], 'hspg' ),
				'deleted'   => _n( '%s donation permanently deleted.', '%s donations permanently deleted.', $bulk_counts['deleted'], 'hspg' ),
				'trashed'   => _n( '%s donation moved to the Trash.', '%s donations moved to the Trash.', $bulk_counts['trashed'], 'hspg' ),
				'untrashed' => _n( '%s donation restored from the Trash.', '%s donations restored from the Trash.', $bulk_counts['untrashed'], 'hspg' ),
			);
		...
		public function add_filters() {
			...
			if ( in_array( $typenow, array( Hspg::DONATION_POST_TYPE ) ) ) {
				hs_admin_view( 'donations-page/filters' );
			}
		}
		...
		public function add_export( $which ) {
			...
			if ( ! current_user_can( 'export_hs_reports' ) ) {
				return;
			}

			/* Add the export button. */
			if ( 'top' == $which && in_array( $typenow, array( Hspg::DONATION_POST_TYPE ) ) ) {
				hs_admin_view( 'donations-page/export' );
			}
		}
		...
		public function modal_forms() {
			...
			/* Add the modal form. */
			if ( in_array( $typenow, array( Hspg::DONATION_POST_TYPE ) ) ) {
				hs_admin_view( 'donations-page/export-form' );
				hs_admin_view( 'donations-page/filter-form' );
			}
		}
		...
		public function load_scripts( $hook ) {
			...
			if ( in_array( $typenow, array( Hspg::DONATION_POST_TYPE ) ) ) {
				...
				wp_enqueue_script( 'hs-admin-tables' );
			}
		}
		...
		public function filter_request_query( $vars ) {
			...
			if ( isset( $_GET['campaign_id'] ) && 'all' != $_GET['campaign_id'] ) {
				$vars['post__in'] = hs_get_table( 'campaign_donations' )->get_donation_ids_for_campaign( $_GET['campaign_id'] );
			}
		...
		public function sort_donations( $clauses ) {
			...
			switch ( $_GET['orderby'] ) {
				case 'amount' :
					$clauses['join'] = "JOIN {$wpdb->prefix}hs_campaign_donations cd ON cd.donation_id = $wpdb->posts.ID ";
		...
		protected function get_status_counts() {
			if ( ! isset( $this->status_counts ) ) {
				...
				$status_counts = Hs_Donations::count_by_status( $args );
