 ...
 *
 * @package   Hspg/Admin/Hs_Campaign_Meta_Boxes
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Campaign_Meta_Boxes' ) ) :

	/**
	 * Hs_Campaign_Meta_Boxes class.
	 *
	 * @final
	 * @since 1.0.0
	 */
	final class Hs_Campaign_Meta_Boxes {

		/**
		 * The single instance of this class.
		 *
		 * @var     Hs_Campaign_Meta_Boxes|null
		 */
		...
		private function __construct() {
			$this->meta_box_helper = new Hs_Meta_Box_Helper( 'hs-campaign' );
		}
		...
		public function add_meta_boxes() {
			$meta_boxes = array_merge(
				...
				array(
					array(
						...
						'title'    => __( 'Campaign Settings', 'hspg' ),
						'context'  => 'normal',
						'priority' => 'high',
			 ...
			 */
			$meta_boxes = apply_filters( 'hs_campaign_meta_boxes', $meta_boxes );
		...
		public function campaign_form_top( $post ) {
			if ( Hspg::CAMPAIGN_POST_TYPE == $post->post_type ) {
				do_meta_boxes( Hspg::CAMPAIGN_POST_TYPE, 'campaign-top', $post );
			}
		}
		...
		public function get_campaign_settings_panels() {
			$panels = hspg()->campaign_fields()->get_sections( 'admin' );
			...
			foreach ( $panels as $section => $label ) {
				...
				if ( 'campaign-donation-options' == $section ) {
					 ...
					 */
					$panels[ $section ]['fields'] = apply_filters( 'hs_campaign_donation_options_fields', $panels[ $section ]['fields'] );
				}
			}
			 ...
			 */
			return apply_filters( 'hs_campaign_settings_panels', $panels );
		}
		...
		public function save_campaign( $campaign_id, WP_Post $post ) {
			...
			foreach ( $meta_keys as $key ) {
				...
				/**
				 * This filter is deprecated. Use hs_sanitize_campaign_meta{$key} instead.
				 *
				 * @deprecated 1.7.0
				 *
				 * @since 1.4.12 Deprecated.
				 */
				$value = apply_filters( 'hs_sanitize_campaign_meta', $value, $key, $submitted, $campaign_id );

				/**
				 * Filter this meta value. The filter hook is
				 * hs_sanitize_campaign_meta{$key}. For example,
				 * for _campaign_end_date the filter hook will be:
				 * hs_sanitize_campaign_meta_campaign_end_date
				 *
				 * @since 1.4.12
				 *
				 ...
				 * @param int   $campaign_id The campaign ID.
				 */
				$data[ $key ] = apply_filters( 'hs_sanitize_campaign_meta' . $key, $value, $submitted, $campaign_id );
			}

			$processor = new Hs_Campaign_Processor( $data, $meta_keys );
			$processor->save_meta();
			 ...
			 */
			do_action( 'hs_campaign_save', $post );
		}
		...
		public function set_default_post_content( $data ) {
			if ( Hspg::CAMPAIGN_POST_TYPE != $data['post_type'] ) {
				return $data;
			}
		...
		public function campaign_enter_title( $placeholder, WP_Post $post ) {
			if ( 'campaign' == $post->post_type ) {
				$placeholder = __( 'Enter campaign title', 'hspg' );
			}
		...
		public function post_messages( $messages ) {
			...
			$messages[ Hspg::CAMPAIGN_POST_TYPE ] = array(
				// Unused. Messages start at index 1.
				0 => '',
				1 => sprintf(
					__( 'Campaign updated. <a href="%s">View Campaign</a>', 'hspg' ),
					esc_url( get_permalink( $post_ID ) )
				),
				2 => __( 'Custom field updated.', 'hspg' ),
				3 => __( 'Custom field deleted.', 'hspg' ),
				4 => __( 'Campaign updated.', 'hspg' ),
				5 => isset( $_GET['revision'] )
					? sprintf( __( 'Campaign restored to revision from %s', 'hspg' ), wp_post_revision_title( (int) $_GET['revision'], false ) )
					: false,
				6 => sprintf(
					__( 'Campaign published. <a href="%s">View Campaign</a>', 'hspg' ),
					esc_url( get_permalink( $post_ID ) )
				),
				7 => __( 'Campaign saved.', 'hspg' ),
				8 => sprintf(
					__( 'Campaign submitted. <a target="_blank" href="%s">Preview Campaign</a>', 'hspg' ),
					esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) )
				),
				9 => sprintf(
					__( 'Campaign scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview Campaign</a>', 'hspg' ),
					date_i18n( __( 'M j, Y @ G:i', 'hspg' ),strtotime( $post->post_date ) ),
					esc_url( get_permalink( $post_ID ) )
				),
				10 => sprintf(
					__( 'Campaign draft updated. <a target="_blank" href="%s">Preview Campaign</a>', 'hspg' ),
					esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) )
				),
			);
		 ...
		 * using the `hs_campaign_meta_boxes` filter.
		 *
		 * @since  1.6.0
		 *
		...
		private function add_legacy_meta_boxes( $panels ) {
			foreach ( apply_filters( 'hs_campaign_meta_boxes', array() ) as $panel ) {
		...
		public function get_campaign_meta_keys() {
			 ...
			 */
			return apply_filters( 'hs_campaign_meta_keys', $keys );
		}
		...
		private function sanitize_campaign_field( $field, $key ) {
			...
			$campaign = hs_get_campaign( $_GET['post'] );
