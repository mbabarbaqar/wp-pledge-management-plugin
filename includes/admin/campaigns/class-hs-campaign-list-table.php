 ...
 *
 * @package   Hspg/Classes/Hs_Campaign_List_Table
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Campaign_List_Table' ) ) :

	/**
	 * Hs_Campaign_List_Table class.
	 *
	 * @since 1.5.0
	 */
	final class Hs_Campaign_List_Table {
		 ...
		 * @since 1.5.0
		 *
		 * @var   Hs_Campaign_List_Table|null
		 */
		...
		public function dashboard_columns() {
			...
			return apply_filters(
				'hs_campaign_dashboard_column_names',
				array(
					'cb'       => '<input type="checkbox"/>',
					'ID'       => __( '#', 'hspg' ),
					'title'    => __( 'Title', 'hspg' ),
					'author'   => __( 'Creator', 'hspg' ),
					'donated'  => __( 'Donations', 'hspg' ),
					'end_date' => __( 'End Date', 'hspg' ),
					'status'   => __( 'Status', 'hspg' ),
					'date'     => __( 'Date Created', 'hspg' ),
				)
			);
		}
		...
		public function dashboard_column_item( $column_name, $post_id ) {
			$campaign = hs_get_campaign( $post_id );
			...
			switch ( $column_name ) {
				...
				case 'donated':
					$display = hs_format_money( $campaign->get_donated_amount() );
					break;
			 ...
			 * @param Hs_Campaign $campaign    The Hs_Campaign object.
			 */
			echo apply_filters( 'hs_campaign_column_display', $display, $column_name, $post_id, $campaign );
		}
		...
		public function bulk_messages( $bulk_messages, $bulk_counts ) {
			$bulk_messages[ Hspg::CAMPAIGN_POST_TYPE ] = array(
				'updated'   => _n( '%d campaign updated.', '%d campaigns updated.', $bulk_counts['updated'], 'hspg' ),
				'locked'    => ( 1 == $bulk_counts['locked'] )
								? __( '1 campaign not updated, somebody is editing it.' )
								: _n( '%s campaign not updated, somebody is editing it.', '%s campaigns not updated, somebody is editing them.', $bulk_counts['locked'], 'hspg' ),
				'deleted'   => _n( '%s campaign permanently deleted.', '%s campaigns permanently deleted.', $bulk_counts['deleted'], 'hspg' ),
				'trashed'   => _n( '%s campaign moved to the Trash.', '%s campaigns moved to the Trash.', $bulk_counts['trashed'], 'hspg' ),
				'untrashed' => _n( '%s campaign restored from the Trash.', '%s campaigns restored from the Trash.', $bulk_counts['untrashed'], 'hspg' ),
			);
		...
		public function add_export( $which ) {
			...
			if ( 'top' == $which && $this->is_campaigns_page() ) {
				hs_admin_view( 'campaigns-page/export' );
			}
		}
		...
		public function add_filters() {
			...
			if ( in_array( $typenow, array( Hspg::CAMPAIGN_POST_TYPE ) ) ) {
				hs_admin_view( 'campaigns-page/filters' );
			}
		}
		...
		public function modal_forms() {
			if ( $this->is_campaigns_page() ) {
				hs_admin_view( 'campaigns-page/filter-form' );
				hs_admin_view( 'campaigns-page/export-form' );
			}
		}
		...
		public function filter_request_query( $vars ) {
			 ...
			 */
			return apply_filters( 'hs_filter_campaigns_list_request_vars', $vars );
		}
		...
		private function is_campaigns_page() {
			...
			return in_array( $typenow, array( Hspg::CAMPAIGN_POST_TYPE ) );
		}
	}
