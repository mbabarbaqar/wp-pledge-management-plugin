...
$sections    = hs_get_admin_settings()->get_sections();
...
if ( $show_return ) {
	...
	$return_tab_url = apply_filters(
		'hs_settings_return_tab_url',
		add_query_arg(
			array( 'tab' => $active_tab ),
			admin_url( 'admin.php?page=hs-settings' )
		),
		$active_tab,
		$group
	);
...
ob_start();
?>
<div id="hs-settings" class="wrap">
	<h1 class="screen-reader-text"><?php echo get_admin_page_title(); ?></ha>
	<h2 class="nav-tab-wrapper">
		<?php foreach ( $sections as $tab => $name ) : ?>
			<a href="<?php echo esc_url( add_query_arg( array( 'tab' => $tab ), admin_url( 'admin.php?page=hs-settings' ) ) ); ?>" class="nav-tab <?php echo $active_tab == $tab ? 'nav-tab-active' : ''; ?>"><?php echo $name; ?></a>
		<?php endforeach ?>
	</h2>
	<?php if ( $show_return ) : ?>
	...
	<?php
		 ...
		 */
		do_action( 'hs_before_admin_settings', $group );
	?>
	<form method="post" action="options.php">
		<table class="form-table">
		<?php
			settings_fields( 'hs_settings' );

			hs_do_settings_fields( 'hs_settings_' . $group, 'hs_settings_' . $group );
		?>
		</table>
		<?php
			/**
			 ...
			 */
			echo apply_filters( 'hs_settings_button_' . $group, get_submit_button( null, 'primary', 'submit', true, null ) );
		?>
	</form>
	<?php
		/**
		 ...
		 * @param string $group The settings group we are viewing.
		 */
		do_action( 'hs_after_admin_settings', $group );
	?>
</div>
<?php
echo ob_get_clean();
