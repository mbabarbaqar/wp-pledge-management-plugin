 ...
 *
 * @package   Hspg/Forms/Hs_Admin_Form_View
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Admin_Form_View' ) ) :
	 ...
	 * @since 1.5.0
	 */
	class Hs_Admin_Form_View implements Hs_Form_View_Interface {
		...
		public function render_field( $field, $key, $args = array() ) {
			...
			$field['wrapper_id']    = 'hs-' . $field['id'] . '-wrap';
			...
			return hs_admin_view( $field['view'], $field );
		}
		...
		protected function get_field_class( $field ) {
			$classes = array(
				'hs-metabox-wrap',
				'hs-' . $this->get_field_type( $field ) . '-wrap',
			);
		...
		protected function use_default_field_template( $field_type ) {
			...
			$default_field_types = apply_filters(
				'hs_default_template_field_types',
				array(
					'text',
					'email',
					'password',
