 ...
 *
 * @package   Hspg/Classes/Hs_Admin_Notices
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Admin_Notices' ) ) :
	 ...
	 * @since 1.4.6
	 */
	class Hs_Admin_Notices extends Hs_Notices {
		...
		public function render() {
			foreach ( hs_get_admin_notices()->get_notices() as $type => $notices ) {
		...
		public function render_notice( $notice, $type, $dismissible = false, $notice_key = '' ) {
			if ( ! isset( $this->script_enqueued ) ) {
				if ( ! wp_script_is( 'hs-admin-notice' ) ) {
					wp_enqueue_script( 'hs-admin-notice' );
				}

				$this->script_enqueued = true;
			}

			$class = 'notice hs-notice';
		...
		public function shutdown() {
			set_transient( 'hs_notices', $this->notices );
		}
		...
		public function load_notices() {
			$this->notices = get_transient( 'hs_notices' );
