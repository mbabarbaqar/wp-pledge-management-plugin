 ...
 *
 * @package   Hspg/Classes/Hs_Donation_Processor
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Donation_Processor' ) ) :

	/**
	 * Hspg Donation Processor.
	 *
	 * @since  1.0.0
	 */
	class Hs_Donation_Processor {

		/**
		 * The single instance of this class.
		 *
		 * @var     Hs_Donation_Processor|null
		 */
		...
		protected function __construct() {
			$this->campaign = hs_get_current_campaign();
		}
		...
		/**
		 * Destroy the Hs_Donation_Processor instance.
		 *
		 ...
		 * @since  1.0.0
		 *
		 * @return Hs_Campaign|false False if no campaign is set. Campaign object otherwise.
		 */
		...
		public static function add_donation_to_session() {
			...
			$nonce = $_POST['hs-donate-now'];

			if ( ! wp_verify_nonce( $nonce, 'hs-donate' ) ) {
			...
			hs_get_session()->add_donation( $processor->get_campaign()->ID, 0 );

			$donations_url = hs_get_permalink( 'campaign_donation_page', array( 'campaign_id' => $processor->get_campaign()->ID ) );
		...
		public function process_donation() {
			 ...
			 * @param Hs_Donation_Form      $form      The Donation Form object.
			 */
			do_action( 'hs_before_process_donation_form', $processor, $form );
			 ...
			 */
			if ( ! apply_filters( 'hs_validate_donation_form_submission_gateway', true, $gateway, $values ) ) {
				return false;
			}
			 ...
			 */
			set_transient( 'hs_donation_' . hs_get_session()->get_session_id(), $this );

			/**
			 * We check whether the gateway is compatible with version 1.3, since Hspg 1.3
			...
			if ( $this->gateway_is_130_compatible( $gateway ) ) {
				 ...
				 * @return mixed
				 */
				return apply_filters( 'hs_process_donation_' . $gateway, true, $this->donation_id, $processor );
				 ...
				 * @param Hs_Donation_Processor $processor   The Donation Processor object.
				 */
				do_action( 'hs_process_donation_' . $gateway, $this->donation_id, $processor );

				/**
				 * If we get this far, forward the user through to the receipt page.
				 */
				wp_safe_redirect( hs_get_permalink( 'donation_receipt_page', array( 'donation_id' => $this->donation_id ) ) );
		...
		public static function ajax_process_donation_form_submission() {
			if ( ! isset( $_POST['campaign_id'] ) ) {
				wp_send_json_error( new WP_Error( 'missing_campaign_id', __( 'Campaign ID was not found. Unable to create donation.', 'hspg' ) ) );
			}
			...
			if ( $processor->donation_processed_successfully( $result ) ) {
				...
				if ( empty( $errors ) ) {
					$errors = array( __( 'Unable to process donation.', 'hspg' ) );
				}
		...
		public static function make_donation_streamlined() {
			...
			/* Validate the form submission and retrieve the values. */
			$form = new Hs_Donation_Amount_Form( $campaign );

			/**
			 * @hook hs_before_process_donation_amount_form
			 */
			do_action( 'hs_before_process_donation_amount_form', $processor, $form );
			...
			if ( $form->validate_submission() ) {
				...
				hs_get_session()->add_donation( $submitted['campaign_id'], $submitted['amount'], $period );

				/**
				 * @hook hs_after_process_donation_amount_form
				 */
				do_action( 'hs_after_process_donation_amount_form', $processor, $submitted );
			}

			$redirect_url = hs_get_permalink( 'campaign_donation_page', array( 'campaign_id' => $campaign->ID ) );
		...
		public function save_donation( array $values ) {
			 ...
			 */
			$this->donation_data = apply_filters( 'hs_donation_values', $values );

			if ( ! $this->get_campaign_donations_data() ) {
				hs_get_deprecated()->doing_it_wrong(
					__METHOD__,
					__( 'A donation cannot be inserted without an array of campaigns being donated to.', 'hspg' ),
					'1.0.0'
				);
				return 0;
			}
			...
			if ( ! $this->is_valid_user_data() ) {
				hs_get_deprecated()->doing_it_wrong(
					__METHOD__,
					__( 'A donation cannot be inserted without valid user data.', 'hspg' ),
					'1.0.0'
				);
				return 0;
			}
			 ...
			 * @since 1.0.0
			 *
			 * @param Hs_Donation_Processor $this The Processor instance.
			 */
			do_action( 'hs_before_save_donation', $this );
			...
			if ( is_wp_error( $donation_id ) ) {
				hs_get_notices()->add_errors_from_wp_error( $donation_id );
				return 0;
			}

			if ( 0 == $donation_id ) {
				hs_get_notices()->add_error( __( 'We were unable to save the donation. Please try again.', 'hspg' ) );
				return 0;
			}
			...
			if ( empty( $log_note ) ) {
				$log_note = isset( $values['ID'] ) && 0 !== $values['ID'] ? __( 'Payment attempted.', 'hspg' ) : __( 'Donation created.', 'hspg' );
			}
			 ...
			 * @param Hs_Donation_Processor $this        The Processor instance.
			 */
			do_action( 'hs_after_save_donation', $donation_id, $this );
		...
		public function save_campaign_donations( $donation_id ) {
			...
			if ( false !== $this->get_donation_data_value( 'ID', false ) ) {
				$records            = hs_get_table( 'campaign_donations' )->get_donation_records( $donation_id );
			...
			foreach ( $campaigns as $campaign ) {
				...
				if ( false !== $campaign_donation_id ) {
					...
					$campaign_donation['amount'] = hs_sanitize_amount( $campaign_donation['amount'], true );

					/* Get our updated campaign donation values. */
					$args             = hs_array_subset( $campaign, array( 'campaign_id', 'campaign_name', 'amount' ) );
					$args['donor_id'] = $donor_id;

					/* If any values have changed, update the record. */
					if ( count( array_diff( $args, $campaign_donation ) ) ) {
						hs_get_table( 'campaign_donations' )->update( $campaign_donation_id, $args, 'campaign_donation_id' );
					}
				} else {
					...
					$campaign_donation_id    = hs_get_table( 'campaign_donations' )->insert( $campaign );
		...
		public function save_donation_meta( $donation_id ) {
			$meta = array(
				...
				'test_mode'        => hs_get_option( 'test_mode', 0 ),
			 ...
			 * @param int                           $donation_id The donation ID.
			 * @param Hs_Donation_Processor $processor   This instance of `Hs_Donation_Processor`.
			 */
			$meta = apply_filters( 'hs_donation_meta', $meta, $donation_id, $this );
			...
			foreach ( $meta as $meta_key => $value ) {
				 ...
				 */
				$value = apply_filters( 'hs_sanitize_donation_meta', $value, $meta_key );
		...
		public function get_donor_id() {
			if ( ! isset( $this->donor_id ) ) {
				...
				if ( array_key_exists( 'email', $user_data ) ) {
					$this->donor_id = hs_get_table( 'donors' )->get_donor_id_by_email( $user_data['email'] );
				}

				/* If we still do not have a donor ID, it means that this is a first-time donor */
				if ( 0 == $this->donor_id ) {
					$user           = new Hs_User( $this->get_donation_data_value( 'user_id', get_current_user_id() ) );
		...
		/**
		 * Returns the donation status. Defaults to hs-pending.
		 *
		 * @since  1.0.0
		 *
		 * @return string
		...
		protected function get_donation_status() {
			$status = $this->get_donation_data_value( 'status', 'hs-pending' );

			if ( ! hs_is_valid_donation_status( $status ) ) {
				$status = 'hs-pending';
			}
		 ...
		 *
		 * @uses   Hs_Donation_Processor::get_redirection_after_gateway_processing()
		 *
		 * @since  1.3.0
		 *
			...
			if ( false == $gateway_processing ) {
				...
				$this->update_donation_log(
					...
					sprintf(
						...
						__( 'Payment failed with errors: %s', 'hspg' ),
						PHP_EOL . implode( PHP_EOL, hs_get_notices()->get_errors() )
					)
				);
		...
		private function get_redirection_after_gateway_processing( $gateway_processing ) {
			if ( false == $gateway_processing ) {
				...
				$redirect_url = hs_get_permalink( 'donation_receipt_page', array( 'donation_id' => $this->donation_id ) );
			}
		...
		protected function parse_donation_data() {
			 ...
			 * @param Hs_Donation_Processor $processor   The donation processer object.
			 */
			return apply_filters( 'hs_donation_values_core', $core_values, $this );
		}
		...
		private function gateway_is_130_compatible( $gateway ) {
			return Hs_Gateways::get_instance()->get_gateway_object( $gateway )->supports( '1.3.0' );
		}
		...
		public function get_ipn_url( $gateway ) {
			hs_get_deprecated()->deprecated_function( __METHOD__, '1.4.0', 'hs_get_ipn_url()' );

			return hs_get_ipn_url( $gateway );
		}
	}
