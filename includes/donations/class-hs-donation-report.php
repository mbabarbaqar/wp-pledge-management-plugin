 ...
 *
 * @package   Hspg/Classes/Hs_Donation_Report
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Donation_Report' ) ) :
	...
	class Hs_Donation_Report {
		...
		private function run_amount_query() {
			if ( empty( $this->args['campaigns'] ) ) {
				return hs_get_table( 'campaign_donations' )->get_total();
			}

			return hs_get_table( 'campaign_donations' )->get_campaign_donated_amount( $this->args['campaigns'] );
		}
		...
		private function parse_report_type( $report_type ) {
			...
			if ( ! in_array( $report_type, $this->types ) ) {
				hs_get_deprecated()->doing_it_wrong(
					...
					sprintf( __( '%s is not a valid donation report type.', 'hspg' ), $report_type ),
					'1.6.0'
				);
