 ...
 *
 * @package   Hspg/Classes/Hs_Donation_Log
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Donation_Log' ) ) :
	...
	class Hs_Donation_Log {
		...
		public function get_log() {
			if ( ! isset( $this->log ) ) {
				 ...
				 * @param Hs_Donation $donation Instance of `Hs_Donation`.
				 */
				$this->log = apply_filters( 'hs_donation_logs', $this->log, hs_get_donation( $this->donation_id ) );

				/* Order the logs by time. */
				usort( $this->log, 'hs_timestamp_sort' );
			}
		...
		private function get_email_log_email_id( $log ) {
			 ...
			 */
			return apply_filters( 'hs_email_id_from_log', $email, $log->meta_key );
		}
		...
		private function get_email_name( $email ) {
			$class = Hs_Emails::get_instance()->get_email( $email );
		...
		private function get_email_action( $email ) {
			if ( false === Hs_Emails::get_instance()->get_email( $email ) ) {
				return false;
			}

			return Hs_Admin::get_instance()->get_donation_actions()->get_action_link( 'resend_' . $email, $this->donation_id );
		}
		...
		private function get_sent_email_log_message( $email_name, $action ) {
			$message = sprintf( __( '%s was sent successfully.', 'hspg' ), $email_name );

			if ( false !== $action ) {
				$message .= sprintf( '&nbsp;<a href="%s">%s</a>', $action, __( 'Resend it now', 'hspg' ) );
			}
		...
		private function get_unsent_email_log_message( $email_name, $action ) {
			$message = sprintf( __( '%s failed to send.', 'hspg' ), $email_name );

			if ( false !== $action ) {
				$message .= sprintf( '&nbsp;<a href="%s">%s</a>', $action, __( 'Retry email send', 'hspg' ) );
			}
