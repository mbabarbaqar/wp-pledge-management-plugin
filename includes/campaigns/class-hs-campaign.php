 ...
 * Campaign model
 *
 * @package   Hspg/Classes/Hs_Campaign
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Campaign' ) ) :
	...
	class Hs_Campaign {
		...
		/**
		 * The Hs_Object_Fields object for this campaign.
		 *
		 * @since 1.6.0
		 *
		 * @var   Hs_Object_Fields
		 */
		protected $fields;
		...
		public function get_meta( $meta_name, $single = true ) {
			 ...
			 * @param  Hs_Campaign $campaign  This instance of `Hs_Campaign`.
			 */
			return apply_filters( 'hs_campaign_get_meta_value', get_post_meta( $this->post->ID, $meta_name, $single ), $meta_name, $single, $this );
		}

		/**
		 * Return the Hs_Object_Fields instance.
		 *
		 * @since  1.6.0
		 *
		 * @return Hs_Object_Fields
		 */
		public function fields() {
			if ( ! isset( $this->fields ) ) {
				$this->fields = new Hs_Object_Fields( hspg()->campaign_fields(), $this );
			}
		...
		public function get_suggested_donations() {
			...
			return apply_filters( 'hs_campaign_suggested_donations', $value, $this );
		}
		...
		public function get_end_date( $date_format = '' ) {
			...
			/* Filter the end date format using the hs_campaign_end_date_format hook. */
			$date_format = apply_filters( 'hs_campaign_end_date_format', $date_format, $this );
		 ...
		 *
		 * @uses hs_campaign_ended          Change the text displayed when there is no time left.
		 ...
		 * @uses hs_campaign_time_left      Change the text displayed when there is time left.
		 *
		 * @since  1.0.0
		 *
		 * @return string
		...
		public function get_time_left() {
			...
			if ( 0 === $seconds_left ) {
				...
				$time_left = apply_filters( 'hs_campaign_ended', __( 'Campaign has ended', 'hspg' ), $this );
				...
				$time_left         = apply_filters(
					...
					sprintf( _n( '%s Minute Left', '%s Minutes Left', $minutes_remaining, 'hspg' ), '<span class="amount time-left minutes-left">' . $minutes_remaining . '</span>' ),
					$this
				);
				...
				$time_left       = apply_filters(
					...
					sprintf( _n( '%s Hour Left', '%s Hours Left', $hours_remaining, 'hspg' ), '<span class="amount time-left hours-left">' . $hours_remaining . '</span>' ),
					$this
				);
				...
				$time_left      = apply_filters(
					...
					sprintf( _n( '%s Day Left', '%s Days Left', $days_remaining, 'hspg' ), '<span class="amount time-left days-left">' . $days_remaining . '</span>' ),
					$this
				);

			}//end if

			return apply_filters( 'hs_campaign_time_left', $time_left, $this );
		}
		...
		public function can_receive_donations() {
			 ...
			 * @param Hs_Campaign $campaign This instance of `Hs_Campaign`.
			 */
			return apply_filters( 'hs_campaign_can_receive_donations', $can, $this );
		}
		...
		public function get_finished_notice() {
			...
			if ( ! $this->has_goal() ) {
				$message = __( 'This campaign ended %s ago', 'hspg' );
			} elseif ( $this->has_achieved_goal() ) {
				$message = __( 'This campaign successfully reached its funding goal and ended %s ago', 'hspg' );
			} else {
				$message = __( 'This campaign failed to reach its funding goal %s ago', 'hspg' );
			}

			return apply_filters( 'hs_campaign_finished_notice', sprintf( $message, '<span class="time-ago">' . human_time_diff( $this->get_end_time() ) . '</span>' ), $this );
		}
		...
		public function get_monetary_goal() {
			...
			return hs_format_money( (string) $this->get_meta( '_campaign_goal' ) );
		}
		...
		public function get_goal_achieved_message( $success = '', $failure = '' ) {
			if ( $this->has_achieved_goal() ) {
				 ...
				 * @param Hs_Campaign $campaign This instance of `Hs_Campaign`.
				 */
				return apply_filters( 'hs_campaign_goal_achievement_success_message', __( 'The campaign achieved its fundraising goal.', 'hspg' ), $this );
			}
			 ...
			 * @param Hs_Campaign $campaign This instance of `Hs_Campaign`.
			 */
			return apply_filters( 'hs_campaign_goal_achievement_failure_message', __( 'The campaign did not reach its fundraising goal.', 'hspg' ), $this );
		}
		...
		public function get_status() {
			 ...
			 * @param Hs_Campaign $campaign    This instance of `Hs_Campaign`.
			 */
			return apply_filters( 'hs_campaign_status', $status, $this->post->post_status, $this );
		}
		...
		public function get_status_key() {
			 ...
			 */
			$ending_soon_threshold = apply_filters( 'hs_campaign_ending_soon_threshold', WEEK_IN_SECONDS );
		...
		public function get_status_tag( $show_achievement = null, $show_active_tag = null ) {
			...
			if ( is_null( $show_achievement ) ) {
				$show_achievement = apply_filters( 'hs_campaign_show_achievement_status_tag', true );
			}

			if ( is_null( $show_active_tag ) ) {
				$show_active_tag = apply_filters( 'hs_campaign_show_active_status_tag', false );
			}
			...
			switch ( $key ) {
				...
				case 'successful':
					$tag = $show_achievement ? __( 'Successful', 'hspg' ) : __( 'Ended', 'hspg' );
					break;

				case 'unsucessful':
					$tag = $show_achievement ? __( 'Unsuccessful', 'hspg' ) : __( 'Ended', 'hspg' );
					break;

				case 'ending':
					$tag = __( 'Ending Soon', 'hspg' );
					break;

				case 'active':
				case 'publish':
					$tag = $show_active_tag ? __( 'Active', 'hspg' ) : '';
					break;
			 ...
			 * @param Hs_Campaign $this This campaign object.
			 */
			return apply_filters( 'hs_campaign_status_tag', $tag, $key, $this );
		}
		...
		public function get_admin_edit_link() {
			$post_type_object = get_post_type_object( Hspg::CAMPAIGN_POST_TYPE );
		...
		public function get_donations() {
			...
			if ( false === $this->donations ) {
				...
				$this->donations = hs_get_table( 'campaign_donations' )->get_donations_on_campaign( $this->ID );
		...
		public function get_donated_amount( $sanitize = false ) {
			...
			if ( false === $this->donated_amount ) {
				$this->donated_amount = hs_get_table( 'campaign_donations' )->get_campaign_donated_amount( $this->ID, false, false );
			...
			if ( ! $sanitize ) {
				$amount = Hs_Currency::get_instance()->sanitize_database_amount( $amount );
			}
			 ...
			 */
			return apply_filters( 'hs_campaign_donated_amount', $amount, $this, $sanitize );
		}
		...
		public function get_donated_amount_formatted() {
			return hs_format_money( $this->get_donated_amount() );
		}
		...
		public function get_donation_summary() {
			...
			if ( $goal ) {
				$ret = sprintf(
					_x( '%1$s donated of %2$s goal', 'amount donated of goal', 'hspg' ),
				...
				$ret = sprintf(
					_x( '%s donated', 'amount donated', 'hspg' ),
					'<span class="amount">' . $currency_helper->get_monetary_amount( $amount ) . '</span>'
				);
			}
			 ...
			 */
			return apply_filters( 'hs_donation_summary', $ret, $this, $amount, $goal );
		}
		...
		public function get_percent_donated() {
			...
			return apply_filters( 'hs_percent_donated', $percent . '%', $percent, $this );
		}
		...
		public function get_donor_count() {
			 ...
			 * @param Hs_Campaign $this  This campaign object.
			 */
			return apply_filters( 'hs_campaign_donor_count', hs_get_table( 'campaign_donations' )->count_campaign_donors( $this->ID ), $this );
		}
		 ...
		 * @since  1.0.0
		 *
		 * @return Hs_Donation_Form_Interface
		 */
		public function get_donation_form() {
			if ( ! isset( $this->donation_form ) ) {
				 ...
				 * @param Hs_Campaign $this  This campaign object.
				 */
				$form_class = apply_filters( 'hs_donation_form_class', 'Hs_Donation_Form', $this );
		...
		public function get_donation_amount_in_session() {
			$donation = hs_get_session()->get_donation_by_campaign( $this->ID );
			 ...
			 * @param Hs_Campaign $this   This campaign object.
			 */
			return apply_filters( 'hs_session_donation_amount', $amount, $this );
		}
		...
		public function get_default_donation_amount() {
			 ...
			 * @param Hs_Campaign $campaign The instance of `Hs_Campaign`.
			 */
			return apply_filters( 'hs_default_donation_amount', 0, $this );
		}
		...
		public function donate_button_template() {
			...
			$display_option = hs_get_option( 'donation_form_display', 'separate_page' );
			...
			switch ( $display_option ) {
				...
				default:
					$template_name = apply_filters( 'hs_donate_button_template', 'campaign/donate-button.php', $this );
			}

			hs_template( $template_name, array( 'campaign' => $this ) );
		}
		...
		public function donate_button_loop_template() {
			...
			switch ( $display_option ) {
				...
				default:
					$template_name = apply_filters( 'hs_donate_button_loop_template', 'campaign-loop/donate-link.php', $this );
			}
		...
		public function get_campaign_creator() {
			return apply_filters( 'hs_campaign_creator', $this->post->post_author, $this );
		}
		...
		public static function sanitize_campaign_goal( $value ) {
			...
			return hs_get_currency_helper()->sanitize_monetary_amount( $value );
		}
		...
		public static function sanitize_campaign_end_date( $value, $submitted = array() ) {
			...
			if ( hspg()->registry()->get( 'i18n' )->decline_months() ) {
				$end_date = $value . ' ' . $end_time;
			} else {
				$end_date = hs_sanitize_date( $value, 'Y-m-d ' . $end_time );
			}
		...
		public static function sanitize_campaign_suggested_donations( $value ) {
			...
			$value = array_filter( $value, array( 'Hs_Campaign', 'filter_suggested_donation' ) );
			...
			foreach ( $value as $key => $suggestion ) {
				$value[ $key ]['amount'] = hs_sanitize_amount( (string) $suggestion['amount'] );
			}
		...
		public static function sanitize_custom_donations( $value, $submitted ) {
			 ...
			 */
			if ( apply_filters( 'hs_campaign_permitted_without_custom_or_suggested', false, $submitted ) ) {
				return $checked;
			}
		...
		public static function flush_donations_cache( $campaign_id ) {
			...
			do_action( 'hs_flush_campaign_cache', $campaign_id );
		}
		...
		private static function get_donations_cache_key( $campaign_id ) {
			return 'hs_campaign_' . $campaign_id . '_donations';
		}
		...
		private static function get_donation_amount_cache_key( $campaign_id ) {
			return 'hs_campaign_' . $campaign_id . '_donation_amount';
		}
		...
		public static function sanitize_meta( $value, $key, $submitted ) {
			hs_get_deprecated()->deprecated_function( __METHOD__, '1.4.2' );
			return apply_filters( 'hs_sanitize_campaign_meta' . $key, $value, $submitted );
		}
	}
