 ...
 *
 * @package   Hspg/Classes/Hs_Campaign_Processor
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Campaign_Processor' ) ) :

	/**
	 * Hs_Campaign_Processor
	 *
	 * @since 1.5.9
	 */
	class Hs_Campaign_Processor {
		...
		public function sanitize( $key, $value ) {
			 ...
			 * @param Hs_Campaign_Processor $processor This processor object.
			 */
			return apply_filters( 'hs_campaign_processor_sanitize_' . $key, $value, $this );
		}
		...
		public function save_core() {
			...
			$data['post_type'] = Hspg::CAMPAIGN_POST_TYPE;
			 ...
			 * @param Hs_Campaign_Processor $processor This processor object.
			 */
			$data = apply_filters( 'hs_campaign_processor_core_data', $data, $this );
		...
		public function save_meta() {
			 ...
			 * @param Hs_Campaign_Processor $processor This processor object.
			 */
			$meta = apply_filters( 'hs_campaign_processor_meta_data', hs_array_subset( $this->args, $this->meta_keys ), $this );
		...
		public function save_taxonomies() {
			 ...
			 */
			$data = apply_filters(
				'hs_campaign_processor_taxonomy_data',
				array(
		...
		protected function get_default_args() {
			...
			return apply_filters(
				'hs_campaign_processor_default_args',
				array(
					'ID'                     => 0,
					'title'                  => '',
					'content'                => '',
		...
		protected function sanitize_goal( $value ) {
			if ( $this->should_skip_sanitizer_for_key( 'goal', array( 'Hs_Campaign', 'sanitize_campaign_goal' ) ) ) {
				return $value;
			}

			return Hs_Campaign::sanitize_campaign_goal( (string) $value );
		}
		...
		protected function sanitize_suggested_donations( $value ) {
			if ( $this->should_skip_sanitizer_for_key( 'suggested_donations', array( 'Hs_Campaign', 'sanitize_campaign_suggested_donations' ) ) ) {
				return $value;
			}

			return Hs_Campaign::sanitize_campaign_suggested_donations( $value );
		}
		...
		protected function sanitize_allow_custom_donations( $value ) {
			if ( $this->should_skip_sanitizer_for_key( 'allow_custom_donations', array( 'Hs_Campaign', 'sanitize_custom_donations' ) ) ) {
				return $value;
			}

			return Hs_Campaign::sanitize_custom_donations( $value, $this->input );
		}
		...
		public function get_suggested_donations_value() {
			...
			return hs_get_campaign( $this->args['ID'] )->get_suggested_donations();
		}
		...
		protected function sanitize_description( $value ) {
			if ( $this->should_skip_sanitizer_for_key( 'description', array( 'Hs_Campaign', 'sanitize_campaign_description' ) ) ) {
				return $value;
			}

			return Hs_Campaign::sanitize_campaign_description( $value );
		}
		...
		protected function sanitize_end_date( $value ) {
			...
			if ( ! $this->is_mysql_date_format( $value ) ) {
				hs_get_deprecated()->doing_it_wrong(
					__METHOD__,
					sprintf( __( 'Campaign processor requires date in YYYY-MM-DD format. Date passed as %s. This message was added in Hspg version 1.5.9.', 'hspg' ), $value ),
					null
				);
		...
		protected function get_meta_key( $key ) {
			 ...
			 */
			return apply_filters( 'hs_campaign_meta_key', $sanitized, $key, $this->campaign_id );
		}
		...
		protected function should_skip_sanitizer_for_key( $key, $callback ) {
			$tag = 'hs_sanitize_campaign_meta_campaign_' . $key;

			/* We already used the sanitizer. Remove this in Hspg 1.6/1.7. */
			if ( did_action( $tag ) ) {
				return true;
			}
			...
			hs_get_deprecated()->doing_it_wrong(
				$tag,
				sprintf( __( '%s is deprecated as a sanitizer on the %s hook. To prevent sanitization, use add_filter( \'hs_campaign_processor_disable_sanitizer_%s\', \'__return_true\' ) instead.', 'hspg' ),
					$method,
					$tag,
					$key
				),
		...
		protected function use_sanitizer( $key ) {
			 ...
			 * @return boolean
			 */
			return ! apply_filters( 'hs_campaign_processor_disable_sanitizer_' . $key, false );
		}
	}
