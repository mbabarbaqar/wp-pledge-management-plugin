 ...
 *
 * @package   Hspg/Classes/Hs_Campaigns
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Campaigns' ) ) :
	...
	class Hs_Campaigns {
		...
		public static function ordered_by_amount( $args = array() ) {
			...
			add_filter( 'posts_join_paged', array( 'Hs_Campaigns', 'join_campaign_donations_table' ) );
			add_filter( 'posts_groupby', array( 'Hs_Campaigns', 'groupby_campaign_id' ) );
			add_filter( 'posts_orderby', array( 'Hs_Campaigns', 'orderby_campaign_donation_amount' ), 10, 2 );

			$query = Hs_Campaigns::query( $args );

			/* Clean up filters */
			remove_filter( 'posts_join_paged', array( 'Hs_Campaigns', 'join_campaign_donations_table' ) );
			remove_filter( 'posts_groupby', array( 'Hs_Campaigns', 'groupby_campaign_id' ) );
			remove_filter( 'posts_orderby', array( 'Hs_Campaigns', 'orderby_campaign_donation_amount' ), 10, 2 );
