...
if ( ! class_exists( 'Hs_Donors_Widget' ) ) :

	/**
	 * Hs_Donors_Widget class.
	 *
	 * @since 1.0.0
	 */
	class Hs_Donors_Widget extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				...
				__( 'Donors', 'hspg' ),
				array(
					'description' => __( 'Display a list of donors.', 'hspg' ),
		...
		public function widget( $args, $instance ) {
			...
			hs_template( 'widgets/donors.php', $view_args );
		}
		...
		public function form( $instance ) {
			...
			$campaigns = Hs_Campaigns::query( array( 'posts_per_page' => -1 ) );
			?>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Title', 'hspg' ); ?>:</label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php _e( 'Number of donors to display', 'hspg' ); ?>:</label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>"><?php _e( 'Order by', 'hspg' ); ?>:</label>
				<select name="<?php echo esc_attr( $this->get_field_name( 'order' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>">
					<option value="recent" <?php selected( 'recent', $args['order'] ); ?>><?php _e( 'Most recent', 'hspg' ); ?></option>
					<option value="amount" <?php selected( 'amount', $args['order'] ); ?>><?php _e( 'Amount donated', 'hspg' ); ?></option>
				</select>
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'campaign_id' ) ); ?>"><?php _e( 'Show donors by campaign', 'hspg' ); ?>:</label>
				<select name="<?php echo esc_attr( $this->get_field_name( 'campaign_id' ) ); ?>">
					...
					<option value="current" <?php selected( 'current', $args['campaign_id'] ); ?>><?php _e( 'Campaign currently viewed', 'hspg' ); ?></option>
					<optgroup label="<?php _e( 'Specific campaign', 'hspg' ); ?>">
			...
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'show_distinct' ) ); ?>"><?php _e( 'Group donations by the same person', 'hspg' ); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'show_avatar' ) ); ?>"><?php _e( 'Show donor\'s avatar', 'hspg' ); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'show_name' ) ); ?>"><?php _e( 'Show donor\'s name', 'hspg' ); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'show_amount' ) ); ?>"><?php _e( 'Show donor\'s donation amount', 'hspg' ); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'show_location' ) ); ?>"><?php _e( 'Show donor\'s location', 'hspg' ); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'hide_if_no_donors' ) ); ?>"><?php _e( 'Hide if there are no donors', 'hspg' ); ?></label>
			</p>
			<?php
			/**
			 ...
			 * @param Hs_Donors_Widget $this This widget instance.
			 */
			do_action( 'hs_donor_widget_settings_bottom', $args, $this );
		}
		...
		public function update( $new_instance, $old_instance ) {
			...
			foreach ( array( 'show_distinct', 'show_avatar', 'show_location', 'show_amount', 'show_name', 'hide_if_no_donors' ) as $key ) {
					$instance[ $key ] = array_key_exists( $key, $instance )
						? hs_sanitize_checkbox( $instance[ $key ] )
						: 0;
			}
			 ...
			 * @param array $old_instance The old settings.
			 */
			return apply_filters( 'hs_donors_widget_update_instance', $instance, $new_instance, $old_instance );
		}
		...
		protected function get_widget_donors( $instance ) {
			$query_args = hs_array_subset( $instance, array( 'number', 'campaign' ) );
			 ...
			 */
			$query_args = apply_filters( 'hs_donors_widget_donor_query_args', $query_args, $instance );
