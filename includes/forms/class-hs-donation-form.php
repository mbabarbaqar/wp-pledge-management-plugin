 ...
 *
 * @package   Hspg/Classes/Hs_Donation_Form
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Donation_Form' ) ) :
	 ...
	 * @since  1.0.0
	 */
	class Hs_Donation_Form extends Hs_Form implements Hs_Donation_Form_Interface {
		 ...
		 * @var   string
		 */
		protected $nonce_action = 'hs_donation';
		 ...
		 * @var   string
		 */
		protected $nonce_name = '_hs_donation_nonce';
		 ...
		 *
		 * @param Hs_Campaign|null $campaign Optional. Campaign receiving the donation, or NULL if
		 *                                           the campaign will be selected in the form.
		 */
		public function __construct( Hs_Campaign $campaign = null ) {
		 ...
		 * @since  1.0.0
		 *
		 * @return Hs_User|false Object if the user is logged in. False otherwise.
		 */
		public function get_user() {
			if ( ! isset( $this->user ) ) {
				$user       = wp_get_current_user();
				$this->user = $user->ID ? new Hs_User( $user ) : false;
			}
		...
		public function get_user_fields() {
			 ...
			 * @param Hs_Donation_Form $form   Instance of `Hs_Donation_Form`.
			 */
			$fields = apply_filters( 'hs_donation_form_user_fields', $this->get_sanitized_donation_fields( 'user' ), $this );
		...
		public function get_meta_fields() {
			 ...
			 * @param Hs_Donation_Form $form   Instance of `Hs_Donation_Form`.
			 */
			$fields = apply_filters( 'hs_donation_form_meta_fields', $this->get_sanitized_donation_fields( 'meta' ), $this );
		...
		public function get_sanitized_donation_fields( $section ) {
			$fields = hspg()->donation_fields()->get_donation_form_fields( $section );
		...
		public function hide_non_required_user_fields( $fields ) {
			if ( ! hs_get_option( 'donation_form_minimal_fields', false ) ) {
				return $fields;
			}
		...
		public function get_user_account_fields() {
			...
			if ( apply_filters( 'hs_donor_usernames', false ) ) {
		...
		public function get_donation_fields() {
			 ...
			 * @param Hs_Donation_Form $form   Instance of `Hs_Donation_Form`.
			 */
			$fields = apply_filters( 'hs_donation_form_donation_fields', $fields, $this );
		...
		public function get_fields() {
			 ...
			 * @param Hs_Donation_Form $form   This instance of `Hs_Donation_Form`.
			 */
			$fields = apply_filters( 'hs_donation_form_fields', $fields, $this );
		...
		public function add_payment_fields( $fields ) {
			$gateways_helper    = hs_get_helper( 'gateways' );
			...
			foreach ( $gateways_helper->get_active_gateways() as $gateway_id => $gateway_class ) {
				 ...
				 * @param Hs_Gateway $gateway        Instance of `Hs_Gateway`.
				 */
				$gateway_fields          = apply_filters( 'hs_donation_form_gateway_fields', $gateway_fields, $gateway );
		...
		public function maybe_show_current_donation_amount() {
			...
			if ( ! $amount ) {
				$content = hs_template_from_session_content(
				...
				$content = hs_template_donation_form_current_amount_text( $amount, $this->get_form_identifier(), $this->get_campaign()->ID );
			}
		 ...
		 * @return array[]
		 */
		public function add_credit_card_fields( $fields, Hs_Gateway $gateway ) {
		...
		public function get_validated_donation_id() {
			...
			if ( hs_user_can_access_donation( $donation_id ) ) {
				return $donation_id;
			}
		...
		public function get_hidden_fields() {
			 ...
			 */
			return apply_filters( 'hs_donation_form_hidden_fields', $fields, $this );
		}
		...
		public function add_hidden_gateway_field( $fields ) {
			$gateways = hs_get_helper( 'gateways' )->get_active_gateways();
		...
		public function add_password_field( $form ) {
			...
			hs_template( 'donation-form/user-login-fields.php' );
		}
		...
		public function validate_submission() {
			 ...
			 * @param Hs_Donation_Form $form This instance of `Hs_Donation_Form`.
			 */
			$this->valid = apply_filters( 'hs_validate_donation_form_submission', $this->valid, $this );
		...
		public function validate_security_check() {
			...
			if ( ! $this->validate_nonce() || ! $this->validate_honeypot() ) {
				hs_get_notices()->add_error(
					__( 'Unfortunately, we were unable to verify your form submission. Please reload the page and try again.', 'hspg' )
				);
			 ...
			 * @param Hs_Donation_Form $form This instance of `Hs_Donation_Form`.
			 */
			return apply_filters( 'hs_validate_donation_form_submission_security_check', $ret, $this );
		}
		...
		public function validate_email() {
			...
			if ( array_key_exists( 'email', $_POST ) && ! is_email( $_POST['email'] ) ) {
				hs_get_notices()->add_error( sprintf(
			 ...
			 * @param Hs_Donation_Form $form This instance of `Hs_Donation_Form`.
			 */
			return apply_filters( 'hs_validate_donation_form_submission_email_check', $ret, $this );
		}
		...
		public function validate_gateway() {
			...
			if ( ! Hs_Gateways::get_instance()->is_valid_gateway( $_POST['gateway'] ) ) {

				hs_get_notices()->add_error( __( 'The gateway submitted is not valid.', 'hspg' ) );
			 ...
			 * @param Hs_Donation_Form $form This instance of `Hs_Donation_Form`.
			 */
			return apply_filters( 'hs_validate_donation_form_submission_gateway_check', $ret, $this );
		...
		public function validate_amount() {
			...
			if ( $minimum > 0 && $amount < $minimum ) {
				...
				$ret = false;
			} elseif ( $minimum == 0 && $amount <= 0 && ! apply_filters( 'hs_permit_0_donation', false ) ) {
				hs_get_notices()->add_error( sprintf(
					__( 'You must donate more than %s.', 'hspg' ),
					hs_format_money( $minimum )
				) );
			 ...
			 * @param Hs_Donation_Form $form This instance of `Hs_Donation_Form`.
			 */
			return apply_filters( 'hs_validate_donation_form_submission_amount_check', $ret, $this );
		...
		public function get_donation_values() {
			...
			if ( 0 !== $values['ID'] ) {
				$values['donation_key'] = hs_get_donation( $values['ID'] )->get_donation_key();
			}
			 ...
			 * @param Hs_Donation_Form $form      This instance of `Hs_Donation_Form`.
			 */
			return apply_filters( 'hs_donation_form_submission_values', $values, $submitted, $this );
		}
		...
		public function maybe_add_terms_conditions_fields( $fields ) {
			$terms_fields = array();
			$show_consent = (int) hs_get_option( 'contact_consent', 0 ) && Hs_Upgrade::get_instance()->upgrade_has_been_completed( 'upgrade_donor_tables' );
			 ...
			 * @param Hs_Donation_Form $form         Instance of `Hs_Donation_Form`.
			 */
			$terms_fields = apply_filters( 'hs_donation_form_terms_fields', $terms_fields, $this );
			...
			return array_merge( $fields, array(
				'terms_fields' => array(
					'legend'   => __( 'Terms and Conditions', 'hspg' ),
					'type'     => 'fieldset',
					'fields'   => $terms_fields,
					'priority' => 80,
				),
		...
		public function get_terms( $terms_page ) {
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.6.14',
				'hs_get_terms_and_conditions'
			);

			return hs_get_terms_and_conditions();
		}
		...
		public function get_parsed_terms_text( $terms_page ) {
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.6.14',
				'hs_get_terms_and_conditions_field_label'
			);

			return hs_get_terms_and_conditions_field_label();
		}
		...
		public function get_parsed_privacy_text( $privacy_page ) {
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.6.14',
				'hs_get_privacy_policy_field_text'
			);

			return hs_get_privacy_policy_field_text();
		}
		...
		public function should_hide_user_fields() {
			 ...
			 * @param Hs_Donation_Form $form        The instance of `Hs_Donation_Form`.
			 */
			return apply_filters( 'hs_hide_fields_for_logged_in_users', $hide_fields, $this );
		}
		...
		public static function get_donation_amount() {
			...
			$amount = hs_get_currency_helper()->sanitize_monetary_amount( (string) $amount );

			return apply_filters( 'hs_donation_form_amount', $amount );
		}
		...
		protected function setup_payment_fields() {
			$active_gateways = hs_get_helper( 'gateways' )->get_active_gateways();
			$has_gateways    = apply_filters( 'hs_has_active_gateways', ! empty( $active_gateways ) );

			/* If no gateways have been selected, display a notice and return the fields */
			if ( ! $has_gateways ) {
				hs_get_notices()->add_error( $this->get_no_active_gateways_notice() );
				return;
			}

			if ( count( $active_gateways ) == 1 ) {
				add_filter( 'hs_donation_form_hidden_fields', array( $this, 'add_hidden_gateway_field' ) );
			}

			add_action( 'hs_donation_form_fields', array( $this, 'add_payment_fields' ) );
		}
		...
		protected function get_credentialed_notice( $notice, $admin_action ) {
			if ( current_user_can( 'manage_hs_settings' ) ) {
				$notice .= '&nbsp;' . $admin_action;
			}
		...
		protected function get_no_active_gateways_notice() {
			$notice = $this->get_credentialed_notice(
				__( 'There are no active payment gateways.', 'hspg' ),
				sprintf( '<a href="%s">%s</a>.', admin_url( 'admin.php?page=hs-settings&tab=gateways' ), _x( 'Enable one now', 'enable payment gateway', 'hspg' ) )
			);

			return apply_filters( 'hs_no_active_gateways_notice', $notice, current_user_can( 'manage_hs_settings' ) );
		}
		...
		protected function check_test_mode() {
			$in_test_mode = hs_get_option( 'test_mode', 0 );

			/* If test mode is enabled, and current user is an admin, display an alert on the form. */
			if ( $in_test_mode && current_user_can( 'manage_hs_settings' ) ) {
				hs_get_notices()->add_error( $this->get_test_mode_active_notice() );
			}
		}
		...
		protected function get_test_mode_active_notice() {
			$notice = $this->get_credentialed_notice(
				__( 'Test mode is active.', 'hspg' ),
				sprintf( '<a href="%s">%s</a>.',
					admin_url( 'admin.php?page=hs-settings&tab=gateways' ),
					__( 'Disable Test Mode', 'hspg' )
				)
			);

			return apply_filters( 'hs_test_mode_active_notice', $notice, current_user_can( 'manage_hs_settings' ) );
		}
		...
		protected function is_missing_required_fields( $required_fields ) {
			...
			if ( is_null( $this->get_submitted_value( 'gateway' ) ) ) {
				...
				hs_get_notices()->add_error( sprintf( '<p>%s</p>',
					__( 'Your donation could not be processed. No payment gateway was selected.', 'hspg' )
				) );
		 ...
		 *
		 * @since  1.0.0
		 * @since  1.5.0 Deprecated. This is handled by `Hs_Public_Form_View` now.
		 *
		 * @param  string|false $custom_template
		 * @return string|false|Hs_Template
		 */
