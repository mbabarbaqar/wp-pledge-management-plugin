 ...
 *
 * @package   Hspg/Forms/Hs_Public_Form_View
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Public_Form_View' ) ) :
	 ...
	 * @since 1.5.0
	 */
	class Hs_Public_Form_View implements Hs_Form_View_Interface {
		 ...
		 */
		public function __construct( Hs_Form $form ) {
			$this->form                   = $form;
		...
		public function enqueue_script() {
			if ( ! wp_script_is( 'hs-forms', 'enqueued' ) ) {
				wp_enqueue_script( 'hs-forms' );
			}
		}
		...
		public function render_honeypot() {
			printf( '<input type="hidden" name="hs_form_id" value="%1$s" autocomplete="off" /><input type="text" name="%1$s" class="hs-hidden" value="" autocomplete="off" />', esc_attr( $this->form->get_form_identifier() ) );
			...
			if ( empty( $fields ) ) {
				...
				if ( empty( $fields ) ) {
					hs_get_deprecated()->doing_it_wrong(
						__METHOD__,
						__( 'There are no fields to render.', 'hspg' ),
						'1.5.0'
					);
					return false;
				}
		...
		public function render_hidden_field( $value, $key ) {
			...
			if ( is_array( $value ) ) {
				...
				$attrs = hs_get_arbitrary_attributes( array( 'attrs' => $args ) );
			}
		...
		public function render_field( $field, $key, $args = array() ) {
			 ...
			 * @param Hs_Form $form          The Hs_Form object.
			 ...
			 */
			do_action( 'hs_form_field', $field, $key, $this->form, $index, null, false );
		...
		protected function get_field_name( $key, $namespace = null, $index = 0 ) {
			 ...
			 * @param int             $index     The current index.
			 */
			return apply_filters( 'hs_form_field_key', $name, $key, $namespace, $this->form, $index );
		}
		 ...
		 * @param  array $field              Field definition.
		 * @param  int   $index              The current index.
		 * @return Hs_Template|false Returns a template if the template file exists. If it doesn't returns false.
		 */
		public function get_field_template( $field, $index ) {
			 ...
			 *
			 * Any callback hooking into this filter should return a `Hs_Template`
			 * instance. Anything else will be ignored.
			 *
			 * @since 1.0.0
			 *
			 * @param false|Hs_Template $template False by default.
			 * @param array                     $field    Field definition.
			 * @param Hs_Form           $form     The Hs_Form object.
			 * @param int                       $index    The current index.
			 */
			$template = apply_filters( 'hs_form_field_template', $template, $field, $this->form, $index );
			...
			if ( ! $this->is_valid_template( $template ) ) {
				$template = new Hs_Template( $this->get_template_name( $field ), false );
			}
		 ...
		 * @return false|Hs_Template False if there is no matching custom template.
		 */
		...
		public function get_template_name( $field ) {
			 ...
			 */
			return apply_filters( 'hs_form_field_template_name', $template_name );
		}
		...
		public function is_valid_template( $template ) {
			return is_object( $template ) && is_a( $template, 'Hs_Template' );
		}
		...
		protected function use_default_field_template( $field_type ) {
			 ...
			 */
			$default_field_types = apply_filters( 'hs_default_template_field_types', array(
				'text',
				'email',
				'password',
				'date',
		...
		public function get_field_classes( $field, $index = 0 ) {
			 ...
			 */
			$classes = apply_filters( 'hs_form_field_classes', $classes, $field, $index );
		...
		public function get_field_type_classes( $type ) {
			...
			switch ( $type ) {
				...
				default:
					$classes[] = 'hs-form-field';
					$classes[] = 'hs-form-field-' . $type;
			}
		...
		public function increment_index( $field, $key = '', $index = null) {
			 ...
			 * @param Hs_Form $form      The form we are displaying.
			 * @param int             $index     The current index.
			 */
			return apply_filters( 'hs_form_field_increment', 1, $field, $key, $this->form, $index );
		}
		...
		protected function init_custom_field_templates() {
			 ...
			 * @since 1.5.0
			 */
			$templates = apply_filters( 'hs_public_form_view_custom_field_templates', array(
