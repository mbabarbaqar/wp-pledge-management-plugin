 ...
 *
 * @version     1.0.0
 * @package     Hspg/Classes/Hs_Ghost_Page
 * @author     HCS
...
if ( ! class_exists( 'Hs_Ghost_Page' ) ) :

	/**
	 * Hs_Ghost_Page class.
	 *
	 * It is used when viewing a frontend Hspg view other than a campaign.
	 *
	 * @since   1.0.0
	 */
	class Hs_Ghost_Page {
		...
		/**
		 * @var     string  $current_view   The current Hspg frontend view. Should never be campaign.
		 * @since   1.0.0
		 */
		...
		public function __construct( $current_view, $args = array() ) {
			if ( 'campaign' == $current_view ) {
				throw new Exception( 'Hs_Ghost_Page should not be constructed when $current_view is "campaign".' );
			}
		...
		private function modify_wp_query() {
			...
			$wp_query->post->post_name             = 'hs-ghost-' . $this->current_view;
