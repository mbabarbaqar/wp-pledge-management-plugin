<?php
/**
 * Main class for setting up the Hspg User Dashboard.
 *
 * @package   Hspg/Classes/Hs_User_Dashboard
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_User_Dashboard' ) ) :
	 ...
	 * @since 1.0.0
	 */
	class Hs_User_Dashboard {

		/**
		 * The single instance of this class.
		 *
		 * @var Hs_User_Dashboard|null
		 */
		...
		private function __construct() {
			...
			do_action( 'hs_user_dashboard_start', $this );
		}
		...
		public function register_menu() {
			register_nav_menu( 'hs-dashboard', __( 'User Dashboard', 'hspg' ) );
		}
		...
		public function get_nav_id() {
			...
			if ( ! isset( $locations['hs-dashboard'] ) ) {
				return 0;
			}

			return wp_get_nav_menu_object( $locations['hs-dashboard'] );
		}
		...
		public function nav_objects() {
			$objects = get_transient( 'hs_user_dashboard_objects' );
			...
			if ( false === $objects ) {
				...
				if ( is_array( $nav_menu_items ) ) {
					foreach ( $nav_menu_items as $nav_menu_item ) {
						switch ( $nav_menu_item->type ) {
							...
							default:
								$identifier = apply_filters( 'hs_nav_menu_object_identifier', $nav_menu_item->object_id, $nav_menu_item );
						}
				...
				set_transient( 'hs_user_dashboard_objects', $objects );
			}
		...
		public function flush_menu_object_cache( $menu_id ) {
			...
			/* If $nav_menu is not set that means the location 'hs-dashboard' is not checked. */
		...
		public function in_nav() {
			...
			if ( ! isset( $this->on_user_dashboard_page ) ) {
				$current_url = trailingslashit( hs_get_current_url() );
				 ...
				 */
				$this->on_user_dashboard_page = apply_filters( 'hs_is_in_user_dashboard', $this->on_user_dashboard_page, $this->nav_objects() );
			}
		...
		public function load_user_dashboard_template( $template ) {
			 ...
			 */
			if ( false === apply_filters( 'hs_force_user_dashboard_template', false ) ) {
				return $template;
			}
			...
			do_action( 'hs_is_user_dashboard' );

			$new_template = apply_filters( 'hs_user_dashboard_template', 'user-dashboard.php' );
			$template     = hs_get_template_path( $new_template, $template );
