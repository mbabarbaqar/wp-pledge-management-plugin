 ...
 * Hspg Upgrade class.
 *
 * The responsibility of this class is to manage migrations between versions of Hspg.
 *
 * @package   Hspg/Classes/Hs_Upgrade
...
if ( ! class_exists( 'Hs_Upgrade' ) ) :
	...
	class Hs_Upgrade {
		 ...
		 * @var   string
		 */
		protected $upgrade_log_key = 'hs_upgrade_log';
		 ...
		 * @var   string
		 */
		protected $version_key = 'hs_version';
		...
		protected function __construct( $db_version = '', $edge_version = '' ) {
			 ...
			 * extensions that extend Hs_Upgrade.
			 */
			...
			$this->upgrade_actions = array(
				'update_upgrade_system'                   => array(
					'version' => '1.3.0',
					'message' => __( 'Hspg needs to update the database.', 'hspg' ),
					'prompt'  => true,
				),
				'fix_donation_dates'                      => array(
					'version' => '1.3.0',
					'message' => __( 'Hspg needs to fix incorrect donation dates.', 'hspg' ),
					'prompt'  => true,
				),
				'trigger_cron'                            => array(
					'version'  => '1.3.4',
				...
				'remove_duplicate_donors'                 => array(
					'version' => '1.5.0',
					'message' => __( 'Hspg needs to remove duplicate donor records.', 'hspg' ),
					'prompt'  => true,
				),
				...
				'upgrade_donor_tables'                    => array(
					'version' => '1.6.0',
					'message' => __( 'Hspg needs to upgrade its database tables.', 'hspg' ),
					'prompt'  => true,
				),
				...
				'fix_empty_donor_ids'                     => array(
					'version'         => '1.6.5',
					'message'         => __( 'Hspg needs to fix donations saved without a donor ID.', 'hspg' ),
					'prompt'          => true,
		...
		public function populate_upgrade_log_on_install() {
			...
			$log = array(
				'install' => array(
					'time'    => time(),
					'version' => Hspg::VERSION,
					'message' => __( 'Hspg was installed.', 'hspg' ),
				),
			);
		...
		public function add_upgrade_notice() {
			if ( isset( $_GET['page'] ) && 'hs-upgrades' == $_GET['page'] ) {
				return;
			}
			 ...
			 */
			if ( ! Hs_Upgrade::get_instance()->notice_shown_for_upgrade_in_progress() ) {
				$upgrade_progress = $this->upgrade_is_in_progress();

				if ( false !== $upgrade_progress ) {
					return Hs_Upgrade::get_instance()->show_upgrade_in_progress_notice( $upgrade_progress );
				}
			}
				...
				<p>
					...
					printf(
						...
						sprintf(
							...
							__( 'Click <a href="%s">here</a> to start the upgrade.', 'hspg' ),
							esc_url( admin_url( 'index.php?page=hs-upgrades&hs-upgrade=' . $action ) )
						)
					);
					?>
				</p>
...
<?php
		...
		public function do_immediate_upgrades() {
			if ( is_admin() && isset( $_GET['page'] ) && 'hs-upgrades' == $_GET['page'] ) {
				return;
			}
		...
		public function show_upgrade_in_progress_notice( $upgrade_progress ) {
			/* Fixes a bug that incorrectly set the page as hs-upgrade */
			if ( isset( $upgrade_progress['page'] ) && 'hs-upgrade' == $upgrade_progress['page'] ) {
				$upgrade_progress['page'] = 'hs-upgrades';
			}
?>
			<div class="error">
				<p>
					<?php
						printf(
							/* translators: %s: upgrade link */
							__( 'Hspg needs to complete an upgrade that was started earlier. Click <a href="%s">here</a> to continue the upgrade.', 'hspg' ),
...
<?php
		...
		public function trigger_cron( $action ) {
			return Hs_Cron::schedule_events();
		}
		 ...
		 *
		 * @see    https://github.com/Hspg/Hspg/issues/56
		 *
		 * @since  1.3.0
		 *
		 * @return void
		...
		public function update_upgrade_system() {
			if ( ! current_user_can( 'manage_hs_settings' ) ) {
				wp_die( __( 'You do not have permission to do Hspg upgrades', 'hspg' ), __( 'Error', 'hspg' ), array( 'response' => 403 ) );
			}

			ignore_user_abort( true );

			if ( ! hs_is_func_disabled( 'set_time_limit' ) && ! ini_get( 'safe_mode' ) ) {
				@set_time_limit( 0 );
			}
			 ...
			 * @see 	https://github.com/Hspg/Hspg/issues/56
			 */
			$table = new Hs_Campaign_Donations_DB();
			$table->create_table();
		 ...
		 * This upgrade routine was added in 1.3.0
		 *
		 * @see    https://github.com/Hspg/Hspg/issues/58
		 *
		 * @since  1.3.0
		 *
		 * @return void
		...
		public function fix_donation_dates() {
			...
			$donations = get_posts( array(
				...
				'paged' => $step,
				'post_status' => array_keys( hs_get_valid_donation_statuses() ),
			) );
			...
			if ( count( $donations ) ) {
				 ...
				 */
				remove_action( 'save_post_' . Hspg::DONATION_POST_TYPE, array( 'Hs_Email_Donation_Receipt', 'send_with_donation_id' ) );
				remove_action( 'save_post_' . Hspg::DONATION_POST_TYPE, array( 'Hs_Email_New_Donation', 'send_with_donation_id' ) );
		 ...
		 *
		 * @see    https://github.com/Hspg/Hspg/issues/409
		 *
		 * @since  1.5.0
		 *
		 * @global WPDB $wpdb
		...
		public function remove_duplicate_donors() {
			...
			if ( count( $donors ) ) {
				...
				foreach ( $donors as $donor ) {
					...
					$donor_id_placeholders = hs_get_query_placeholders( count( $duplicates ), '%d' );
					$sql                   = "UPDATE {$wpdb->prefix}hs_campaign_donations
						SET donor_id = %d
					...
					if ( false !== $updated ) {
						$sql     = "DELETE FROM {$wpdb->prefix}hs_donors WHERE donor_id IN ( {$donor_id_placeholders} );";
				...
				$redirect = add_query_arg( array(
					'hs-upgrade' => 'remove_duplicate_donors',
					'page'               => 'hs-upgrades',
					'step'               => $step,
					'number'             => $number,
					'total'              => $total,
		 ...
		 * @see    Hs_upgrade::fix_donation_dates()
		 *
		 * @since  1.3.0
		 *
		 * @return void
		...
		public function upgrade_logs() {
			...
			if ( false === $last_log || version_compare( $last_log['to'], '1.1.0', '<' ) ) {
				Hs_Cron::schedule_events(); // 1.1.0 upgrade
		...
		/**
		 * Remove the 'manage_hs_settings' cap from the Campaign Manager role.
		 *
		 * @since  1.4.5
		 *
		 * @global WP_Roles
		...
		public function remove_campaign_manager_cap( $action ) {
			...
			if ( is_object( $wp_roles ) ) {
				$wp_roles->remove_cap( 'campaign_manager', 'manage_hs_settings' );
			}
		...
		public function clear_campaign_amount_donated_transient( $action ) {
			...
			$sql = "DELETE FROM $wpdb->options
					WHERE option_name LIKE '_transient_hs_campaign_%_donation_amount'";
		...
		/**
		 * Prior to 1.4.18, when first installing Hspg all the upgrade details were
		...
		public function set_update_notice_transient( $upgrade, $action ) {
			set_transient( 'hs_' . $upgrade['notice'] . '_notice', 1 );
		...
		protected function upgrade_is_in_progress() {
			$doing_upgrade = get_option( 'hs_doing_upgrade', false );
		...
		/**
		 * Finish an upgrade. This clears the hs_doing_upgrade setting and updates the log.
		 *
		 * @since  1.3.0
		 *
		...
		protected function finish_upgrade( $upgrade, $redirect_url = '' ) {
			delete_option( 'hs_doing_upgrade' );
		...
		protected function update_tables() {
			try {
				hs_get_table( 'campaign_donations' )->create_table();
				hs_get_table( 'donors' )->create_table();
				return true;
			} catch ( Exception $e ) {
				return false;
			}
		...
		public function upgrade_donor_tables() {
			try {
				hs_get_table( 'donors' )->create_table();
				hs_get_table( 'donormeta' )->create_table();
		...
		public function fix_donor_role_caps() {
			...
			add_role( 'donor', __( 'Donor', 'hspg' ), array( 'read' => true ) );
		...
		protected function has_empty_donor_ids() {
			return 0 < $this->count_empty_donor_id_donations( get_option( 'hs_skipped_donations_with_empty_donor_id', array() ) );
		}
		...
		protected function count_empty_donor_id_donations( $skipped = array() ) {
			if ( empty( $skipped ) ) {
				return hspg()->get_db_table( 'campaign_donations' )->count_donations_by_donor( 0, true );
			}

			global $wpdb;

			$placeholders = hs_get_query_placeholders( count( $skipped ), '%d' );
			$sql          = "SELECT COUNT(*)
            				 FROM {$wpdb->prefix}hs_campaign_donations
            				 WHERE donor_id = 0
		...
		public function fix_empty_donor_ids() {
			...
			$step    = array_key_exists( 'step', $_GET ) ? absint( $_GET['step'] ) : 1;
			$number  = 20;
			$skipped = get_option( 'hs_skipped_donations_with_empty_donor_id', array() );
			...
			$donations_table = hspg()->get_db_table( 'campaign_donations' );
			$donors_table    = hspg()->get_db_table( 'donors' );
			...
			if ( count( $donations ) ) {
				...
				update_option( 'hs_skipped_donations_with_empty_donor_id', $skipped );

				$step++;

				$redirect = add_query_arg( array(
					'hs-upgrade' => 'fix_empty_donor_ids',
					'page'               => 'hs-upgrades',
					'step'               => $step,
					'number'             => $number,
					'total'              => $total,
		 ...
		 *
		 * We're keeping these functions since Hspg add-ons extend this
		...
		public static function upgrade_from( $db_version, $edge_version ) {
			if ( self::requires_upgrade( $db_version, $edge_version ) ) {
				new Hs_Upgrade( $db_version, $edge_version );
			}
		}
		...
		protected function do_upgrades() {
			/**
			 * Before Hspg 1.3, upgrades were in a simple key=>value
			 * array format.
			 *
