 ...
 *
 * This is part of the EDD Software Licensing suite. Just renamed here for Hspg.
 *
 * @author  Pippin Williamson
 * @version 1.6
 */
class Hs_Plugin_Updater {
	private $api_url  = '';
	...
	/**
	 * This disables the default update row shown for Hspg extensions,
	...
	public function show_update_notification( $file, $plugin ) {
		...
		$version_info = Hs_Licenses::get_instance()->get_version_info( $this->name );
		...
		if ( version_compare( $this->version, $version_info->new_version, '<' ) ) {
			...
			switch ( $version_info->download_link ) {
				...
				default:
					printf(
						__( '<p>There is a new version of %1$s available. <a target="_blank" class="thickbox" href="%2$s">View version %3$s details</a> or <a href="%4$s">update now</a>.</p>', 'hspg' ),
	...
	public function show_changelog() {
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( __( 'You do not have permission to install plugin updates', 'hspg' ), __( 'Error', 'hspg' ), array( 'response' => 403 ) );
		}

		$version_info = Hs_Licenses::get_instance()->get_version_info( $_REQUEST['plugin'] );
