 ...
 *
 * @package     Hspg/Classes/Hs_Licenses
 * @version     1.4.20
 * @author     HCS
...
if ( ! class_exists( 'Hs_Licenses' ) ) :
	...
	class Hs_Licenses {
		...
		/* @var string */
		const UPDATE_URL = 'https://www.wphspg.com';

		/**
		 * The single instance of this class.
		 *
		 * @var Hs_Licenses|null
		 */
		...
		private function __construct() {
			...
			add_action( 'hs_deactivate_license', array( $this, 'deactivate_license' ) );
		...
		/**
		 * Checks for any Hspg extensions with updates.
		 *
		 * @since  1.4.0
		 *
		 ...
		 * @param  string          $package  Package URL. For Hspg extensions without a
		...
		public function set_upgrader_error_message( $reply, $package, $upgrader ) {
			...
			if ( 'missing_license' == $package ) {
				if ( $ajax_skin ) {
					$message = sprintf( __( 'You have not activated your license key. Activate your license to update: %s', 'hspg' ),
						admin_url( 'admin.php?page=hs-settings&tab=licenses' )
					);
				} else {
					$message = sprintf( __( 'You have not activated your license key. <a href="%s" target="_top">Activate your license to update.</a>', 'hspg' ),
						admin_url( 'admin.php?page=hs-settings&tab=licenses' )
					);
				}
			...
			if ( false !== strpos( $package, 'expired_license:' ) ) {
				...
				if ( $ajax_skin ) {
					$message = sprintf( __( 'Your license has expired. Renew your license key: %s', 'hspg' ),
						$renewal_link
					);
				} else {
					$message = sprintf( __( 'Your license has expired. <a href="%s" target="_blank">Renew your license key.</a>', 'hspg' ),
			...
			switch ( $version_info['package'] ) {
				...
				case 'missing_license':
					return array(
						...
						'description' => sprintf(
							__( '<p>There is a new version of %1$s available but you have not activated your license. <a target="_top" href="%2$s">Activate your license</a> or <a target="_blank" class="thickbox" href="%3$s">view version %4$s details</a>.</p>', 'hspg' ),
				...
				case 'expired_license':
					$base_renewal_url = isset( $version_info['renewal_link'] ) ? $version_info['renewal_link'] : 'https://www.wphspg.com/account';

					return array(
						'reason_code' => 'expired_license',
						'description' => sprintf(
							__( '<p>There is a new version of %1$s available but your license has expired. <a target="_blank" href="%2$s">Renew your license</a> or <a target="_blank" class="thickbox" href="%3$s">view version %4$s details</a>.</p>', 'hspg' ),
							esc_html( $version_info['name'] ),
				...
				default:
					...
					foreach ( $version_info['requirements'] as $type => $details ) {
						...
						switch ( $type ) {
							...
							case 'hspg':
								if ( version_compare( hspg()->get_version(), $details, '<' ) ) {
									$messages[] = esc_html(
										sprintf(
											__( 'Requires Hspg version %s or greater.' ),
											$details
										)
									);
								}
						...
						return array(
							...
							'description' => sprintf(
								__( '<p>There is a new version of %1$s available but you are missing the following minimum requirements:</p>%2$s', 'hspg' ),
								esc_html( $version_info['name'] ),
		 ...
		 * @param  string $url       The base URL where the plugin is licensed. Defaults to Hs_Licenses::UPDATE_URL.
		 * @return void
		 */
		public function register_licensed_product( $item_name, $author, $version, $file, $url = false ) {
			if ( ! $url ) {
				$url = Hs_Licenses::UPDATE_URL;
			}
			...
			new Hs_Plugin_Updater( $url, $file, array(
				'version'   => $version,
				'license'   => $license,
				'item_name' => $item_name,
				'author'    => $author,
		...
		public function get_licenses() {
			if ( ! isset( $this->licenses ) ) {
				$this->licenses = hs_get_option( 'licenses', array() );
			}
		...
		public function deactivate_license() {
			if ( ! wp_verify_nonce( $_REQUEST['_nonce'], 'license' ) ) {
				wp_die( esc_attr__( 'Cheatin\' eh?!', 'hspg' ) );
			}
			...
			if ( false === $product_key ) {
				wp_die( esc_attr__( 'Missing product key', 'hspg' ) );
			}
			...
			if ( ! $product || ! $this->has_valid_license( $product_key ) ) {
				wp_die( esc_attr__( 'This product is not valid or does not have a valid license key.', 'hspg' ) );
			}

			$license = $this->get_license( $product_key );

			/* Data to send to wphspg.com to deactivate the license. */
			$api_params = array(
		...
		protected function flush_update_cache() {
			wp_cache_delete( 'plugin_versions', 'hspg' );
		...
		/**
		 * Return the latest versions of Hspg plugins.
		 *
		 * @since  1.4.0
		 *
		 * @return array
		...
		protected function get_versions() {
			$versions = wp_cache_get( 'plugin_versions', 'hspg' );
			...
			if ( false === $versions ) {
				...
				$response = wp_remote_post(
					Hs_Licenses::UPDATE_URL . '/edd-api/versions-v2/',
					array(
						'sslverify' => false,
						'timeout'   => 15,
						'body'      => array(
				...
				wp_cache_set( 'plugin_versions', $versions, 'hspg' );
			}//end if
