...
if ( ! class_exists( 'Hs_Form' ) ) :
	...
	abstract class Hs_Form {
		 ...
		 * @var   string
		 */
		protected $nonce_action = 'hs_form';
		 ...
		 * @var   string
		 */
		protected $nonce_name = '_hs_form_nonce';
		...
		protected function attach_hooks_and_filters() {
			hs_get_deprecated()->doing_it_wrong(
				...
				sprintf(
					...
					__( 'None of the callbacks are required as of Hspg 1.5; use `Hs_Public_Form_View` instead. Issue encountered in %s class.' ), get_class( $this )
				),
				'1.5.0'
			);

			add_action( 'hs_form_before_fields', array( $this, 'render_error_notices' ) );
			add_action( 'hs_form_before_fields', array( $this, 'add_hidden_fields' ) );
			add_action( 'hs_form_field', array( $this, 'render_field' ), 10, 6 );
			add_filter( 'hs_form_field_increment', array( $this, 'increment_index' ), 10, 2 );
		}
		 ...
		 * @since  1.5.0
		 *
		 * @param  Hs_Form_View_Interface $form_view An object implementing the `Hs_Form_View_Interface`.
		 * @return void
		 */
		public function set_form_view( Hs_Form_View_Interface $form_view ) {
			$this->view = $form_view;
		}
		...
		public function view() {
			if ( ! isset( $this->view ) ) {
				$this->view = new Hs_Public_Form_View( $this );
			}
		...
		public function get_hidden_fields() {
			 ...
			 * @param Hs_Form $form   This instance of `Hs_Form`.
			 */
			return apply_filters( 'hs_form_hidden_fields', array(
				$this->nonce_name   => wp_create_nonce( $this->nonce_action ),
				'_wp_http_referer'  => wp_unslash( $_SERVER['REQUEST_URI'] ),
				'hs_action' => $this->form_action,
			), $this );
		}
		...
		public function validate_nonce() {
			...
			if ( ! $validated ) {
				hs_get_notices()->add_error( __( 'Unable to submit form. Please try again.', 'hspg' ) );

				if ( defined( 'CHARITABLE_DEBUG' ) && CHARITABLE_DEBUG ) {
					error_log(
						sprintf(
							/* translators: %1$s: nonce name; %2$s: nonce action; %3$s: submitted nonce */
							__( 'Nonce verification failed for nonce with name "%1$s" and action "%2$s". Submitted nonce is "%3$s".', 'hspg' ),
							$this->nonce_name,
							$this->nonce_action,
		...
		public function validate_honeypot() {
			...
			if ( ! isset( $submitted['hs_form_id'] ) ) {
				return true;
			}

			$form_id = $submitted['hs_form_id'];
		...
		public function check_required_fields( $fields, $submitted = array() ) {
			...
			foreach ( $this->get_required_fields( $fields ) as $key => $field ) {
				...
				$exists = apply_filters( 'hs_required_field_exists', $exists, $key, $field, $submitted, $this );
			...
			$missing = apply_filters( 'hs_form_missing_fields', $missing, $this, $fields, $submitted );
			...
			if ( count( $missing ) ) {
				...
				hs_get_notices()->add_error(
					sprintf(
						...
						__( 'There were problems with your form submission. The following required fields were not filled out:', 'hspg' ),
						$missing_fields
					)
				);
		...
		public function get_submitted_values() {
			if ( ! isset( $this->submitted ) ) {
				$this->submitted = apply_filters( 'hs_form_submitted_values', $_POST, $this );
			}
		...
		protected function get_file_overrides( $file_key, $overrides = array() ) {
			...
			$defaults = array(
				'test_form' => false,
				'mimes'     => apply_filters( 'hs_file_' . $file_key . '_allowed_mimes', $allowed_mimes ),
			);
		...
		public function get_template_name( $field ) {
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.5.0',
				'Hs_Public_Form_View::get_template_name()'
			);
		...
		protected function is_valid_template( $template ) {
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.5.0',
				'Hs_Public_Form_View::is_valid_template()'
			);
		...
		protected function use_default_field_template( $field_type ) {
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.5.0',
				'Hs_Public_Form_View::use_default_field_template()'
			);
		...
		public function increment_index( $increment, $field ) {
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.5.0',
				'Hs_Public_Form_View::increment_index()'
			);
			 ...
			 * is called directly.
			 */
			remove_filter( 'hs_form_field_increment', array( $this, 'increment_index' ), 10, 2 );
		 ...
		 *
		 * @param  Hs_Form $form Form object.
		...
		public function render_error_notices( $form ) {
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.5.0',
				'Hs_Public_Form_View::render_notices()'
			);
		...
		public function add_hidden_fields( $form ) {
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.5.0',
				'Hs_Public_Form_View::render_hidden_fields()'
			);
		 ...
		 *
		 * @since  1.0.0
		 * @since  1.5.0 Deprecated. Use `Hs_Public_Form_View` instead.
		 * @since  1.5.4 Added $from_template parameter for backwards compatibility purposes.
		 *               Anytime this hook is called from a template using the pre-Hspg 1.5
		 *               approach of rendering fields (i.e. do_action( 'hs_form_field' )),
		...
		public function render_field( $field, $key, $form, $index = 0, $namespace = null, $from_template = true ) {
			...
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.5.0',
				'Hs_Public_Form_View::render_field()'
			);
