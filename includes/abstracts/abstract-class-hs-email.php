...
if ( ! class_exists( 'Hs_Email' ) ) :
	 ...
	 * @since 1.0.0
	 */
	abstract class Hs_Email implements Hs_Email_Interface {
		...
		public function get_from_name() {
			return wp_specialchars_decode( hs_get_option( 'email_from_name', get_option( 'blogname' ) ) );
		}
		...
		public function get_from_address() {
			return hs_get_option( 'email_from_email', get_option( 'admin_email' ) );
		}
		...
		public function get_content_type() {
			 ...
			 * @param Hs_Email $email        This instance of `Hs_Email`.
			 */
			return apply_filters( 'hs_email_content_type', 'text/html', $this );
		}
		...
		public function get_headers() {
			 ...
			 */
			return apply_filters( 'hs_email_headers', $this->headers, $this );
		}
		...
		public function is_preview() {
			return isset( $_GET['hs_action'] ) && 'preview_email' == $_GET['hs_action'];
		}
		...
		public function email_settings() {
			if ( func_num_args() ) {
				hs_get_deprecated()->deprecated_argument(
					__METHOD__,
					'1.5.0',
					__( 'The `$settings` parameter is no longer used.', 'hspg' )
				);
			}
			...
			$email_settings = array(
				...
				'subject'       => array(
					'type'     => 'text',
					'title'    => __( 'Email Subject Line', 'hspg' ),
					'help'     => __( 'The email subject line when it is delivered to recipients.', 'hspg' ),
					'priority' => 6,
					'class'    => 'wide',
				...
				'headline'      => array(
					'type'     => 'text',
					'title'    => __( 'Email Headline', 'hspg' ),
					'help'     => __( 'The headline displayed at the top of the email.', 'hspg' ),
					'priority' => 10,
					'class'    => 'wide',
				...
				'body'          => array(
					'type'     => 'editor',
					'title'    => __( 'Email Body', 'hspg' ),
					'help'     => sprintf(
						'%s <div class="hs-shortcode-options">%s</div>',
						__( 'The content of the email that will be delivered to recipients. HTML is accepted.', 'hspg' ),
			 ...
			 *
			 * hs_settings_fields_emails_email
			 *
			 ...
			 */
			return apply_filters( 'hs_settings_fields_emails_email_' . $this->get_email_id(), $email_settings );
		}
		...
		public function add_recipients_field( $settings = array() ) {
			...
			$settings['recipient'] = array(
				'type'     => 'text',
				'title'    => __( 'Recipients', 'hspg' ),
				'help'     => __( 'A comma-separated list of email address that will receive this email.', 'hspg' ),
				'priority' => 4,
				'class'    => 'wide',
		...
		public function send() {
			 ...
			 * @param Hs_Email $email The email object.
			 */
			do_action( 'hs_before_send_email', $this );
			 ...
			 * @param Hs_Email $email The email object.
			 */
			do_action( 'hs_after_send_email', $this, $sent );
		...
		public static function resend( $object_id, $args = array() ) {
			hs_get_deprecated()->doing_it_wrong(
				__METHOD__,
				__( 'A `resend` method has not been defined for this class.', 'hspg' ),
				'1.5.0'
			);
		...
		public static function can_be_resent( $object_id, $args = array() ) {
			hs_get_deprecated()->doing_it_wrong(
				__METHOD__,
				__( 'A `can_be_resent` method has not been defined for this class.', 'hspg' ),
				'1.5.0'
			);
		...
		public function preview() {
			 ...
			 * @since 1.0.0
			 *
			 * @param Hs_Email $this Email object.
			 */
			do_action( 'hs_before_preview_email', $this );
			 ...
			 * @param Hs_Email $this Email object.
			 */
			do_action( 'hs_after_preview_email', $this );
		...
		public function get_body() {
			 ...
			 */
			return apply_filters( 'hs_email_body', $body, $this );
		}
		...
		public function get_headline() {
			 ...
			 * @param Hs_Email $email    Instance of `Hs_Email`.
			 */
			return apply_filters( 'hs_email_headline', $headline, $this );
		}
		...
		public function has_valid_donation() {
			if ( is_null( $this->donation ) || ! is_a( $this->donation, 'Hs_Donation' ) ) {
				hs_get_deprecated()->doing_it_wrong(
					__METHOD__,
					__( 'You cannot send this email without a donation!', 'hspg' ),
					'1.0.0'
				);
		...
		public function has_valid_campaign() {
			if ( is_null( $this->campaign ) || ! is_a( $this->campaign, 'Hs_Campaign' ) ) {
				hs_get_deprecated()->doing_it_wrong(
					__METHOD__,
					__( 'You cannot send this email without a campaign!', 'hspg' ),
					'1.0.0'
				);
		...
		protected function build_email() {
			...
			hs_template( 'emails/header.php', array( 'email' => $this ) );

			hs_template( 'emails/body.php', array( 'email' => $this ) );

			hs_template( 'emails/footer.php', array( 'email' => $this ) );
			 ...
			 * @param Hs_Email $email   Instance of `Hs_Email`.
			 */
			return apply_filters( 'hs_email_message', $message, $this );
		}
		...
		protected function get_option( $key, $default ) {
			return hs_get_option( array( 'emails_' . $this->get_email_id(), $key ), $default );
		}
		...
		protected function get_shortcode_options() {
			$fields = new Hs_Email_Fields( $this );
			ob_start();
?>
			<p><?php _e( 'The following options are available with the <code>[hs_email]</code> shortcode:', 'hspg' ) ?></p>
			<ul>
			<?php foreach ( $fields->get_fields() as $key => $field ) : ?>
				<li><strong><?php echo $field['description'] ?></strong>: [hs_email show=<?php echo $key ?>]</li>
			<?php endforeach ?>
			</ul>
...
<?php
			 ...
			 * @param Hs_Email $email This instance of `Hs_Email`.
			 */
			return apply_filters( 'hs_email_shortcode_options_text', $html, $this );
		}
		 ...
		 *
		 * @param  Hs_Email $email  Email object.
		 * @return boolean
		 */
		protected function is_current_email( Hs_Email $email ) {
		 ...
		 * @return array[]
		 */
		public function add_donation_content_fields( $fields, Hs_Email $email ) {
			return $fields;
		}
		 ...
		 * @return array[]
		 */
		public function add_preview_donation_content_fields( $fields, Hs_Email $email ) {
			return $fields;
		}
		 ...
		 * @return array[]
		 */
		public function add_campaign_content_fields( $fields, Hs_Email $email ) {
			return $fields;
		}
		 ...
		 * @return array[]
		 */
		public function add_preview_campaign_content_fields( $fields, Hs_Email $email ) {
			return $fields;
		}
	}
