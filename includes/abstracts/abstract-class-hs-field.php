...
if ( ! class_exists( 'Hs_Field' ) ) :
	 ...
	 */
	abstract class Hs_Field implements Hs_Field_Interface {
		...
		public function set( $key, $setting, $value ) {
			...
			if ( ! is_array( $arg ) ) {
				hs_get_deprecated()->doing_it_wrong(
					...
					sprintf(
						_x( 'Attempting to set an argument setting for a non-array argument. Argument: %1$s; Setting: %2$s', 'argument key', 'hspg' ),
						$key,
						$setting
					),
					'1.6.0'
