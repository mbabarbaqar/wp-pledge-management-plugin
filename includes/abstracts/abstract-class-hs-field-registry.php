<?php
/**
 * Base Hs_Field_Registry model.
 *
 * @package   Hspg/Classes/Hs_Field_Registry
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Field_Registry' ) ) :
	 ...
	 * @since 1.6.0
	 */
	abstract class Hs_Field_Registry implements Hs_Field_Registry_Interface {
		 ...
		 * @return Hs_Field|false Instance of `Hs_Field` if the field
		...
		public function order_fields() {
			foreach ( $this->forms as $key ) {
				$fields = array_filter( $this->fields, array( new Hs_Field_Filter( $key ), 'is_not_false' ) );

				new Hs_Field_Sorter( $fields, $this, $key );
			}
		}
		 ...
		 * @return array
		 */
		protected function parse_form_settings( array $settings, Hs_Field $field ) {
