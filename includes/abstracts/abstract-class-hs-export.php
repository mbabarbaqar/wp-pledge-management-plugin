 ...
 *
 * @package   Hspg/Classes/Hs_Export
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Export' ) ) :
	...
	abstract class Hs_Export {
		...
		public function can_export() {
			return (bool) apply_filters( 'hs_export_capability', current_user_can( 'export_hs_reports' ), $this );
		}
		...
		protected function map_data( $data ) {
			...
			foreach ( $this->columns as $key => $label ) {
				 ...
				 *
				 * Note that this filter applies to all Hspg exports
				 ...
				 */
				$value = apply_filters( 'hs_export_data_key_value', $this->set_custom_field_data( $value, $key, $data ), $key, $data );
				...
				if ( version_compare( PHP_VERSION, '5.3.0', '>=' ) ) {
					 ...
					 * @param mixed  $data  The raw input data.
					 */
					$value = apply_filters( 'hs_export_' . static::EXPORT_TYPE . '_data_key_value', $value, $key, $data );
				}
		...
		protected function print_headers() {
			...
			header( 'Content-Disposition: attachment; filename=hs-export-' . $export . '-' . date( 'm-d-Y' ) . '.csv' );
			header( 'Expires: 0' );
		}
