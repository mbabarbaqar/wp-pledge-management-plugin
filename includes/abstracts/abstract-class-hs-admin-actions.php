 ...
 *
 * @package   Hspg/Classes/Hs_Admin_Actions
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Admin_Actions' ) ) :
	 ...
	 * @since 1.5.0
	 */
	abstract class Hs_Admin_Actions implements Hs_Admin_Actions_Interface {
		...
		public function add_action_fields( $object_id, $action_id ) {
			...
			echo '<div class="hs-action-fields" style="display: none;" data-type="' . esc_attr( $action_id ) . '">' . $fields . '</div>';
		}
		...
		public function do_action( $action, $object_id, $args = array() ) {
			if ( ! $this->action_exists( $action ) ) {
				...
				return new WP_Error( sprintf( __( 'Action "%s" is not registered.', 'hspg' ), $action ) );
			}
			...
			$action_hook = sprintf( 'hs_%s_admin_action_%s', $this->get_type(), $action );
			 ...
			 *
			 * hs_{type}_admin_action_{action key}
			 *
			 * @since 1.5.0
			 *
