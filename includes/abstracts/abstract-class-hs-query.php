<?php
/**
 * An abstract base class defining common methods used by Hspg queries.
 *
 * @package   Hspg/Classes/Hs_Query
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Query' ) ) :
	 ...
	 * @since  1.0.0
	 */
	abstract class Hs_Query implements Iterator {
		...
		public function set( $key, $value ) {
			$this->args[ $key ] = apply_filters( 'hs_query_sanitize_argument_' . $key, $value, $this );
		}
		...
		public function get_query() {
			if ( ! isset( $this->query ) ) {
				 ...
				 * @since 1.0.0
				 *
				 * @param Hs_Query The `Hs_Query` instance.
				 */
				do_action( 'hs_pre_query', $this );
		...
		public function get_found_items() {
			 ...
			 * @param Hs_Query $query                 The `Hs_Query` instance.
			 */
			$found_items_query = apply_filters( 'hs_found_items_query', 'SELECT FOUND_ROWS()', $this );
		 ...
		 *
		 * This is used instead of Hs_Query::fields and is only appropriate
		...
		public function select_count() {
			 ...
			 * @param Hs_Query $query The `Hs_Query` instance.
			 */
			return apply_filters( 'hs_select_count_fields', "COUNT(DISTINCT {$wpdb->posts}.ID)", $this );
		}
		...
		public function fields() {
			 ...
			 * @param Hs_Query $query The `Hs_Query` instance.
			 */
			return apply_filters( 'hs_query_fields', "$found_rows {$wpdb->posts}.ID", $this );
		}
		...
		public function from() {
			 ...
			 * @param Hs_Query $query The `Hs_Query` instance.
			 */
			return apply_filters( 'hs_query_from', "FROM $wpdb->posts", $this );
		}
		...
		public function join() {
			 ...
			 * @param Hs_Query $query The `Hs_Query` instance.
			 */
			return apply_filters( 'hs_query_join', '', $this );
		}
		...
		public function where() {
			 ...
			 * @param Hs_Query $query This query object.
			 */
			return apply_filters( 'hs_query_where', 'WHERE 1=1 ', $this );
		}
		...
		public function groupby() {
			 ...
			 * @param Hs_Query $query This query object.
			 */
			return apply_filters( 'hs_query_groupby', '', $this );
		}
		...
		public function orderby() {
			 ...
			 * @param Hs_Query $query The `Hs_Query` instance.
			 */
			return apply_filters( 'hs_query_orderby', "ORDER BY {$wpdb->posts}.ID", $this );
		}
		...
		public function order() {
			 ...
			 * @param Hs_Query $query The `Hs_Query` instance.
			 */
			return apply_filters( 'hs_query_order', $this->get( 'order', 'DESC' ), $this );
		}
		...
		public function limit() {
			 ...
			 * @param Hs_Query $query The `Hs_Query` instance.
			 */
			return apply_filters( 'hs_query_limit', sprintf( 'LIMIT %d', $this->get( 'number', 20 ) ), $this );
		}
		...
		public function offset() {
			 ...
			 * @param Hs_Query $query The `Hs_Query` instance.
			 */
			return apply_filters( 'hs_query_offset', sprintf( 'OFFSET %d', $offset ), $this );
		}
		...
		public function where_donation_plan_is_in( $where_statement ) {
			...
			$placeholders = $this->get_where_in_placeholders( $donation_plan, 'hs_validate_absint', '%d' );
		...
		public function where_campaign_is_in( $where_statement ) {
			...
			$placeholders = $this->get_where_in_placeholders( $this->get( 'campaign', 0 ), 'hs_validate_absint', '%d' );
		...
		public function where_status_is_in( $where_statement ) {
			...
			$placeholders = $this->get_where_in_placeholders( $status, 'hs_is_valid_donation_status', '%s' );
		...
		public function where_donor_id_is_in( $where_statement ) {
			...
			$placeholders = $this->get_where_in_placeholders( $donor_id, 'hs_validate_absint', '%d' );
		...
		public function where_user_id_is_in( $where_statement ) {
			...
			$placeholders = $this->get_where_in_placeholders( $user_id, 'hs_validate_absint', '%d' );
		...
		public function join_campaign_donations_table_on_campaign( $join_statement ) {
			global $wpdb;
			return $join_statement . " INNER JOIN {$wpdb->prefix}hs_campaign_donations cd ON cd.campaign_id = $wpdb->posts.ID ";
		}
		...
		public function join_campaign_donations_table_on_donation( $join_statement ) {
			global $wpdb;
			return $join_statement . " INNER JOIN {$wpdb->prefix}hs_campaign_donations cd ON cd.donation_id = $wpdb->posts.ID ";
		}
		...
		public function join_donors_table( $join_statement ) {
			global $wpdb;
			return $join_statement . " INNER JOIN {$wpdb->prefix}hs_donors d ON d.donor_id = cd.donor_id ";
		}
		...
		protected function get_placeholders( $count = 1, $placeholder = '%s' ) {
			return hs_get_query_placeholders( $count, $placeholder );
		}
	}
