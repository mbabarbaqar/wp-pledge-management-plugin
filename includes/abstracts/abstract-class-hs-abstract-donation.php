 ...
 * Donation model.
 *
 * @package   Hspg/Classes/Hs_Donation
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Abstract_Donation' ) ) :
	 ...
	 */
	abstract class Hs_Abstract_Donation {
		...
		/**
		 * Hs_Donation donation data for the donation plan this donation is part of.
		 *
		 * @since 1.4.5
		 *
		 * @var false|Hs_Donation
		 */
		...
		/**
		 * The Hs_Donor object of the person who donated.
		 *
		 * @var Hs_Donor
		 */
		protected $donor;
		...
		/**
		 * The Hs_Donation_Log object for this donation.
		 *
		 * @since 1.5.4
		 *
		 * @var   Hs_Donation_Log
		...
		/**
		 * The Hs_Object_Fields object for this donation.
		 *
		 * @since 1.5.0
		 *
		...
		public function fields() {
			if ( ! isset( $this->fields ) ) {
				$this->fields = new Hs_Object_Fields( hspg()->donation_fields(), $this );
			}
		...
		public function get_number() {
			 ...
			 */
			return apply_filters( 'hs_donation_number', $this->donation_id );
		}
		 ...
		 * @since  1.0.0
		 *
		 * @return Hs_Campaign_Donations_DB
		 */
		public function get_campaign_donations_db() {
			if ( ! isset( $this->campaign_donations_db ) ) {
				$this->campaign_donations_db = new Hs_Campaign_Donations_DB();
			}
		...
		public function get_total_donation_amount( $sanitize = false ) {
			...
			if ( $sanitize ) {
				$amount = Hs_Currency::get_instance()->sanitize_monetary_amount( (string) $amount );
			}
		...
		public function get_amount_formatted() {
			return hs_format_money( $this->get_total_donation_amount() );
		}
			...
			foreach ( $this->get_campaign_donations() as $campaign ) {
				...
				if ( ! isset( $links[ $campaign->campaign_id ] ) ) {
					...
					$links[ $campaign->campaign_id ] = sprintf(
						...
						sprintf( '%s %s', _x( 'Go to', 'go to campaign', 'hspg' ), get_the_title( $campaign->campaign_id ) ),
		 ...
		 * @uses   wp_list_pluck
		 * @uses   Hs_Donation::get_campaign_donations
		 *
		 * @since  1.4.2
		 *
		 ...
		 *
		 * @uses   Hs_Donation::get_campaign_categories_donated_to
		 *
		 * @since  1.5.0
		 *
		 * @return string
		...
		protected function get_donation_summary_line( $campaign_donation ) {
			$line_item = sprintf(
				...
				hs_format_money( $campaign_donation->amount ),
				PHP_EOL
			);
			 ...
			 *
			 * @since 1.0.0
			 * @since 1.5.0 Filter was previously defined in Hs_Email::get_donation_summary()
			 *              and included an array of shortcode arguments and a Hs_Email object.
			 *
			 * @param string $line_item         The line of text.
			 * @param object $campaign_donation A campaign donation object.
			 */
			return apply_filters( 'hs_donation_summary_line_item_email', $line_item, $campaign_donation );
		}
		...
		public function get_gateway_label() {
			 ...
			 * @param Hs_Donation $donation This `Hs_Donation` instance.
			 */
			return apply_filters( 'hs_donation_gateway_label', $label, $this );
		}
		 ...
		 * @since  1.0.0
		 *
		 * @return false|Hs_Gateway
		 */
		public function get_gateway_object() {
			$object = hs_get_helper( 'gateways' )->get_gateway_object( $this->get_gateway() );
		 ...
		 *
		 * @since  1.0.0
		 * @since  1.5.0 Deprecated $label argument. Use Hs_Donation::get_status_label() instead.
		 *
		...
		public function get_status( $label = false ) {
			...
			if ( $label ) {
				hs_get_deprecated()->deprecated_argument(
					__METHOD__,
					'1.5.0',
					__( 'The $label argument is deprecated. Use Hs_Donation::get_status_label() instead.', 'hspg' )
				);
		...
		public function has_status( $status ) {
			 ...
			 * @param Hs_Donation $donation   This instance of `Hs_Donation`.
			 * @param string|array        $status     The status we are checking for.
			 */
			return apply_filters( 'hs_donation_has_status', $has_status, $this, $status );
		}
		...
		public function get_donor() {
			if ( ! isset( $this->donor ) ) {
				$this->donor = new Hs_Donor( $this->get_donor_id(), $this->donation_id );
			}
		...
		public function get_test_mode( $text = true ) {
			...
			if ( $text ) {
				return $test_mode ? __( 'Yes', 'hspg' ) : __( 'No', 'hspg' );
			}
		...
		public function get_contact_consent() {
			...
			if ( ! $this->get_donor()->contact_consent ) {
				return $consent ? __( 'Withdrawn', 'hspg' ) : __( 'Not Given', 'hspg' );
			}

			return $consent ? __( 'Given', 'hspg' ) : __( 'Given Later', 'hspg' );
		}
		...
		public function get_donation_meta() {
			$fields = hspg()->donation_fields()->get_meta_fields();
			 ...
			 * @param Hs_Donation $donation This instance of `Hs_Donation`.
			 */
			return apply_filters( 'hs_donation_admin_meta', $meta, $this );
		}
		...
		public function is_from_current_user() {
			...
			if ( hs_get_session()->has_donation_key( $this->get_donation_key() ) ) {
				return true;
			}
		 ...
		 * @since  1.5.4
		 *
		 * @return Hs_Donation_Log
		 */
		public function log() {
			if ( ! isset( $this->log ) ) {
				$this->log = new Hs_Donation_Log( $this->ID );
			}
		...
		public function update_status( $new_status ) {
			...
			if ( false === hs_is_valid_donation_status( $new_status ) ) {
			...
			$message = sprintf(
				...
				__( 'Donation status updated from %1$s to %2$s.', 'hspg' ),
			...
			/**
			 * Handle specific Hspg status transitions.
			 *
			 * @since 1.5.0
			 *
			 * @param Hs_Donation $donation This instance of `Hs_Donation`.
			 */
			do_action( 'hs_donation_status_from_' . $old_status . '_to_' . $new_status, $this );

			/**
			 * Handle specific Hspg status change.
			 *
			 * @since 1.5.0
			 *
			 * @param Hs_Donation $donation   This instance of `Hs_Donation`.
			 ...
			 */
			do_action( 'hs_donation_status_' . $new_status, $this, $old_status );

			/**
			 * Handle all Hspg status changes.
			 *
			 * @since 1.5.0
			 *
			 * @param Hs_Donation $donation   This instance of `Hs_Donation`.
			 ...
			 * @param string              $old_status The previous donation status.
			 */
			do_action( 'hs_donation_status_changed', $this, $new_status, $old_status );
		 ...
		 * @since  1.5.4 Now uses Hs_Donation_Log::add() and returns a result.
		 *
		...
		public function set_gateway_transaction_id( $value ) {
			...
			$value = hs_sanitize_donation_meta( $value, $key );
		 ...
		 * @return array
		 */
		protected function parse_donation_meta_field( Hs_Donation_Field $field ) {
			$value = hs_get_sanitized_donation_field_value( $this->get( $field->field ), $field->field );
		...
		public function get_valid_donation_statuses() {
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.4.0',
				'hs_get_valid_donation_statuses'
			);

			return hs_get_valid_donation_statuses();
		}
		 ...
		 * @deprecated 1.7.0
		 *
		 * @see    hs_is_valid_donation_status
		 *
		 * @since  1.0.0
		 * @since  1.4.0 Deprecated.
		 *
		...
		public function is_valid_donation_status( $status ) {
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.4.0',
				'hs_is_valid_donation_status'
			);

			return hs_is_valid_donation_status( $status );
		}
		...
		public function get_approval_statuses() {
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.4.0',
				'hs_get_approval_statuses'
			);

			return hs_get_approval_statuses();
		}
		...
		public function is_approved_status( $status ) {
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.4.0',
				'hs_is_approved_status'
			);

			return hs_is_approved_status( $status );
		}
		...
		public function sanitize_meta( $value, $key ) {
			hs_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.4.0',
				'hs_sanitize_donation_meta()'
			);

			return hs_sanitize_donation_meta( $value, $key );
		}
		...
		public function flush_campaigns_donation_cache( $donation_id ) {
			...
			return hs_flush_campaigns_donation_cache( $donation_id );
		}
	}
