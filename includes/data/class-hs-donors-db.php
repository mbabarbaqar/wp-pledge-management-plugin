<?php
/**
 * Hspg Donors DB class.
 *
 * @package   Hspg/Classes/Hs_Donors_DB
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Donors_DB' ) ) :
	 ...
	 * @since 1.0.0
	 */
	class Hs_Donors_DB extends Hs_DB {
		...
		public function __construct() {
			...
			$this->table_name = $wpdb->prefix . 'hs_donors';
		}
		...
		public function insert( $data, $type = 'donors' ) {
			if ( ! $this->validate_donor_email( $data ) ) {
				hs_get_notices()->add_error(
					__( 'Unable to insert donor. This email address is already in use.', 'hspg' )
				);
				return 0;
			}
		...
		public function update( $row_id, $data = array(), $where = 'donor_id' ) {
			if ( ! $this->validate_donor_email( $data, $row_id, $where ) ) {
				hs_get_notices()->add_error(
					__( 'Unable to update donor. This email address is already in use.', 'hspg' )
				);
				return false;
			}
		...
		public function maybe_log_consent( $data, $row_id, $where = 'donor_id' ) {
			...
			$log      = new Hs_Donor_Consent_Log( $donor_id );
		 ...
		 * @return int
		 */
		public function count_donors_with_donations( $statuses = array( 'hs-completed' ) ) {
			global $wpdb;
			...
			$sql = "SELECT COUNT( DISTINCT(d.donor_id) )
				FROM {$wpdb->prefix}hs_donors d
				INNER JOIN {$wpdb->prefix}hs_campaign_donations cd ON cd.donor_id = d.donor_id
		...
		public function erase_donor_data( $donor_id ) {
			...
			$placeholders = hs_get_query_placeholders( count( $donor_id ), '%d' );
