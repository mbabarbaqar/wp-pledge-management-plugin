<?php
/**
 * Hspg Campaign Donations DB class.
 *
 * @package   Hspg/Classes/Hs_Campaign_Donations_DB
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Campaign_Donations_DB' ) ) :

	/**
	 * Hs_Campaign_Donations_DB
	 *
	 * @since 1.0.0
	 */
	class Hs_Campaign_Donations_DB extends Hs_DB {
		...
		public function __construct() {
			...
			$this->table_name = $wpdb->prefix . 'hs_campaign_donations';
		}
		...
		public function insert( $data, $type = 'campaign_donation' ) {
			...
			Hs_Campaign::flush_donations_cache( $data['campaign_id'] );
		...
		public static function delete_donation_records( $donation_id ) {
			...
			foreach ( $table->get_campaigns_for_donation( $donation_id ) as $campaign_id ) {
				Hs_Campaign::flush_donations_cache( $campaign_id );
			}
		...
		public function get_total( $statuses = array() ) {
			...
			if ( empty( $statuses ) ) {
				$statuses = hs_get_approval_statuses();
			}
			...
			if ( $this->is_comma_decimal() ) {
				$total = Hs_Currency::get_instance()->sanitize_database_amount( $total );
			}
		 ...
		 *
		 * @uses  Hs_Campaign_Donations_DB::get_campaign_donations_by()
		 *
		 * @since  1.0.0
		 *
		 * @param  int|int[] $donation_id A single donation ID or an array of IDs.
		...
		public function get_campaign_donated_amount( $campaigns, $include_all = false, $sanitize = true ) {
			...
			$statuses = $include_all ? array() : hs_get_approval_statuses();
		 ...
		 *
		 * @uses   Hs_Campaign_Donations_DB::get_distinct_ids()
		 *
		 * @since  1.0.0
		 *
		...
		public function get_donations_report( $args ) {
			...
			if ( isset( $args['status'] ) ) {
				...
				$statuses            = array_keys( hs_get_valid_donation_statuses() );
				$in                  = hs_get_query_placeholders( count( $statuses ), '%s' );
			...
			$sql = "SELECT cd.donation_id, cd.campaign_id, cd.campaign_name, cd.amount, d.email, d.first_name, d.last_name, p.post_date, p.post_content, p.post_status
                    FROM $this->table_name cd
                    INNER JOIN {$wpdb->prefix}hs_donors d
		...
		public function get_donations_summary_by_period( $period = '', $statuses = array() ) {
			...
			$sql = "SELECT COALESCE( SUM( cd.amount ), 0 ) as amount, COUNT( cd.donation_id ) as count
                    FROM {$wpdb->prefix}hs_campaign_donations cd
                    INNER JOIN $wpdb->posts p ON p.ID = cd.donation_id
		...
		private function get_donation_status_clause( $statuses = array() ) {
			...
			$statuses = array_filter( $statuses, 'hs_is_valid_donation_status' );
			$in       = hs_get_query_placeholders( count( $statuses ), '%s' );
		...
		private function get_in_clause_params( $list ) {
			...
			return array( hs_get_query_placeholders( count( $list ), '%d' ), $list );
		}
		...
		private function is_comma_decimal() {
			if ( ! isset( $this->comma_decimal ) ) {
				$this->comma_decimal = Hs_Currency::get_instance()->is_comma_decimal();
			}
		...
		private function sanitize_amounts( $campaign_donation ) {
			$campaign_donation->amount = Hs_Currency::get_instance()->sanitize_database_amount( $campaign_donation->amount );
			...
			switch ( $field ) {
				...
				default:
					hs_get_deprecated()->doing_it_wrong(
						__METHOD__,
						__( 'Field expected to be `campaign`, `campaign_id`, `donation`, `donation_id`, `donor` or `donor_id`.', 'hspg' ),
						'1.4.0'
					);
