 ...
 *
 * @package   Hspg/Classes/Hs_Donation_Field_Registry
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Donation_Field_Registry' ) ) :

	/**
	 * Hs_Donation_Field_Registry
	 *
	 * @since 1.5.0
	 */
	class Hs_Donation_Field_Registry extends Hs_Field_Registry implements Hs_Field_Registry_Interface {
		...
		public function get_donation_form_fields( $section = '' ) {
			if ( ! isset( $this->donation_form_fields ) ) {
				$this->donation_form_fields = array_filter( $this->fields, array( new Hs_Field_Filter( 'donation_form' ), 'is_not_false' ) );
			}
		...
		public function get_meta_fields() {
			return array_filter( $this->fields, array( new Hs_Field_Filter( 'show_in_meta' ), 'is_true' ) );
		}
		...
		public function get_data_type_fields( $data_type ) {
			return array_filter( $this->fields, array( new Hs_Field_Filter( 'data_type', $data_type ), 'is_equal_to' ) );
		}
		...
		public function register_field( Hs_Field_Interface $field ) {
			if ( ! is_a( $field, 'Hs_Donation_Field' ) ) {
				return false;
			}
		...
		public function get_field_value_callback( $field ) {
			...
			switch ( $field->data_type ) {
				case 'user':
					return 'hs_get_donor_meta_value';

				case 'meta':
					return 'hs_get_donation_meta_value';
		 ...
		 * @return array|false
		 */
		protected function get_field_donation_form( Hs_Donation_Field $field ) {
		 ...
		 * @return array
		 */
		protected function get_field_admin_form( Hs_Donation_Field $field ) {
