 ...
 *
 * @package   Hspg/Classes/Hs_Email_Email_Verification
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Email_Email_Verification' ) ) :
	 ...
	 * @since   1.5.0
	 */
	class Hs_Email_Email_Verification extends Hs_Email {
		...
		public function __construct( $objects = array() ) {
			...
			add_filter( 'hs_settings_fields_emails_email_' . self::ID, array( $this, 'add_email_settings' ), 1 );
			 ...
			 */
			$this->name = apply_filters( 'hs_email_email_verification_name', __( 'User: Email Verification', 'hspg' ) );
			...
			return array_merge(
				...
				array(
					'send_after_registration' => array(
						'type'     => 'radio',
						'title'    => __( 'Send after Registration', 'hspg' ),
						'help'     => __( 'Whether to send an email verification message immediately after registration. If this is not set, users are only prompted to verify their email when they try to access their donation history.', 'hspg' ),
						'priority' => 22,
						'class'    => 'wide',
						'default'  => 1,
						'options'  => array(
		...
		public function email_fields() {
			$fields = array(
				'confirmation_url' => array(
					'description' => __( 'The link the user needs to verify their email address.', 'hspg' ),
					'preview'     => add_query_arg(
						array(
							'key'   => '123123123',
							'login' => 'adam123',
						),
						hs_get_permalink( 'email_verification' )
					),
				),
			);
		...
		public function get_confirmation_url() {
			if ( ! isset( $this->confirmation_url ) ) {
				...
				$this->confirmation_url = esc_url_raw( add_query_arg( $args, hs_get_permalink( 'email_verification' ) ) );
			}
		...
		public function get_redirect_url() {
			if ( ! isset( $this->redirect_url ) ) {
				$profile            = hs_get_option( 'profile_page', false );
		...
		public function send() {
			...
			if ( $sent ) {
				update_user_meta( $this->user->ID, '_hs_email_verification_email_send_time', time() );
			}
		...
		public function sent_recently() {
			$send_time = get_user_meta( $this->user->ID, '_hs_email_verification_email_send_time', true );
			 ...
			 */
			$threshold = apply_filters( 'hs_email_verification_resend_threshold_time', 1800 );
		...
		protected function get_default_subject() {
			return __( 'Verify your email for [hs_email show=site_name]', 'hspg' );
		}
		...
		protected function get_default_headline() {
			 ...
			 * @param Hs_Email_Email_Verification $email    This instance of `Hs_Email_Email_Verification`.
			 */
			return apply_filters( 'hs_email_email_verification_default_headline', __( 'Verify your email address', 'hspg' ), $this );
		}
		...
		protected function get_default_body() {
			ob_start();
			?>
<p><?php _e( 'Welcome to [hs_email show=site_name]', 'hspg' ); ?></p>
<p><?php _e( 'To complete your registration, please confirm your email address by clicking the link below:', 'hspg' ); ?></p>
<p><a href="[hs_email show=confirmation_url]">[hs_email show=confirmation_url]</a></p>
			<?php
			/**
			 ...
			 * @param Hs_Email_Email_Verification $email This instance of `Hs_Email_Email_Verification`.
			 */
			return apply_filters( 'hs_email_email_verification_default_body', ob_get_clean(), $this );
		}
