 ...
 * Email Fields class.
 *
 * @package   Hspg/Classes/Hs_Email_Fields
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Email_Fields' ) ) :
	...
	class Hs_Email_Fields {
		 ...
		 */
		public function __construct( Hs_Email $email, $preview = false ) {
			$this->email   = $email;
		...
		public function init_fields() {
			...
			$fields    = array(
				'site_name' => array(
					'description' => __( 'Your website title', 'hspg' ),
					'value'       => $site_name,
					'preview'     => $site_name,
				),
				'site_url'  => array(
					'description' => __( 'Your website URL', 'hspg' ),
					'value'       => $home_url,
					'preview'     => $home_url,
				),
			);
			 ...
			 * automatically.
			 *
			 * This will be removed in or after Hspg 1.8.
			 */
			add_filter( 'hs_email_content_fields', array( $this->email, 'add_donation_content_fields' ), 10, 2 );
			add_filter( 'hs_email_preview_content_fields', array( $this->email, 'add_preview_donation_content_fields' ), 10, 2 );
			add_filter( 'hs_email_content_fields', array( $this->email, 'add_campaign_content_fields' ), 10, 2 );
			add_filter( 'hs_email_preview_content_fields', array( $this->email, 'add_preview_campaign_content_fields' ), 10, 2 );
			 ...
			 * @param Hs_Email $email  Instance of `Hs_Email` type.
			 */
			$fields = apply_filters( 'hs_email_content_fields', $fields, $this->email );

			/**
			 * Finally, if we are previewing the email, parse any preview fields added
			 * through the `hs_email_preview_content_fields` filter, which is
		...
		public function get_field_value( $field, $args ) {
			 ...
			 * @param Hs_Email $email The Email object.
			 */
			return apply_filters( 'hs_email_content_field_value_' . $field, $value, $args, $this->email );
		}
		...
		private function get_object_type_fields( $fields, $object_type ) {
			...
			$class = 'Hs_Email_Fields_' . str_replace( ' ', '_', ucwords( $words ) );
			...
			if ( ! is_a( $object, 'Hs_Email_Fields_Interface' ) ) {
				return $fields;
			}
		...
		/**
		 * Parse preview fields registered through hs_email_preview_content_fields hook.
		 *
		 * @since  1.5.0
		 *
		...
		private function parse_preview_fields( $fields ) {
			 ...
			 * @param Hs_Email $email  The instance of `Hs_Email`.
			 */
			$preview_values = apply_filters( 'hs_email_preview_content_fields', array(), $this->email );
