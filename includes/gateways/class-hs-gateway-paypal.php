 ...
 *
 * @package   Hspg/Classes/Hs_Gateway_Paypal
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Gateway_Paypal' ) ) :
	 ...
	 * @since  1.0.0
	 */
	class Hs_Gateway_Paypal extends Hs_Gateway {
		...
		public function __construct() {
			$this->name = apply_filters( 'hs_gateway_paypal_name', __( 'PayPal', 'hspg' ) );

			$this->defaults = array(
				'label' => __( 'PayPal', 'hspg' ),
			);
		...
		public function gateway_settings( $settings ) {
			return array_merge( $settings, array(
				'paypal_email'             => array(
					'type'     => 'email',
					'title'    => __( 'PayPal Email Address', 'hspg' ),
					'priority' => 6,
					'help'     => __( 'Enter the email address for the PayPal account that should receive donations.', 'hspg' ),
				),
				'sandbox_paypal_email'     => array(
					'type'     => 'email',
					'title'    => __( 'Sandbox PayPal Email Address', 'hspg' ),
					'priority' => 7,
					'help'     => __( 'Enter the email address for the Sandbox PayPal account that should receive test donations.', 'hspg' ),
				),
				'transaction_mode'         => array(
					'type'     => 'radio',
					'title'    => __( 'PayPal Transaction Type', 'hspg' ),
					'priority' => 8,
					'options'  => array(
						'donations' => __( 'Donations', 'hspg' ),
						'standard'  => __( 'Standard Transaction', 'hspg' ),
					),
					'default'  => 'donations',
					'help'     => sprintf( '%s<br /><a href="%s" target="_blank">%s</a>',
						__( 'PayPal offers discounted fees to registered non-profit organizations. You must create a PayPal Business account to apply.', 'hspg' ),
						'https://www.paypal.com/us/webapps/mpp/donations',
						__( 'Find out more.', 'hspg' )
					),
				),
				'disable_ipn_verification' => array(
					'type'     => 'checkbox',
					'title'    => __( 'Disable IPN Verification', 'hspg' ),
					'priority' => 10,
					'default'  => 0,
					'help'     => __( 'If you are having problems with donations not getting marked as Paid, disabling IPN verification might fix the problem. However, it is important to be aware that this is a <strong>less secure</strong> method for verifying donations.', 'hspg' ),
				),
				'api'                      => array(
					'type'     => 'heading',
					'title'    => __( 'API Settings', 'hspg' ),
					'priority' => 20,
				),
				'api_description'          => array(
					'type'     => 'content',
					'content'  => '<div class="hs-settings-notice">'
						. '<p>' . __( 'API credentials are necessary to process PayPal refunds and subscription cancellations from inside WordPress. To find your credentials:', 'hspg' ) . '</p>'
						. '<ul>'
						. '<li>' . sprintf(
							/* translators: %s: link */
							__( '<strong>Live</strong>: Go to %s.', 'hspg' ),
						...
						. '<li>' . sprintf(
							/* translators: %s: link */
							__( '<strong>Sandbox</strong>: Go to %s, click on <em>Profile</em> for the Seller account and open the <em>API keys</em> tab.', 'hspg' ),
				...
				'api_username'             => array(
					'title'    => __( 'Live API Username', 'hspg' ),
					'type'     => 'text',
					'priority' => 30,
					'default'  => '',
				),
				'api_password'             => array(
					'title'    => __( 'Live API Password', 'hspg' ),
					'type'     => 'password',
					'priority' => 40,
					'default'  => '',
				),
				'api_signature'            => array(
					'title'    => __( 'Live API Signature', 'hspg' ),
					'type'     => 'password',
					'priority' => 50,
					'default'  => '',
				),
				'sandbox_api_username'     => array(
					'title'       => __( 'Sandbox API Username', 'hspg' ),
					'type'        => 'text',
					'description' => __( 'Create sandbox accounts and obtain API credentials from within your <a href="http://developer.paypal.com">PayPal developer account</a> or read more about it in the <a href="https://developer.paypal.com/docs/classic/api/apiCredentials/#creating-an-api-signature" target="_blank">documentation</a>.', 'hspg' ),
					'priority'    => 60,
					'default'     => '',
				),
				'sandbox_api_password'     => array(
					'title'    => __( 'Sandbox API Password', 'hspg' ),
					'type'     => 'password',
					'priority' => 70,
					'default'  => '',
				),
				'sandbox_api_signature'    => array(
					'title'    => __( 'Sandbox API Signature', 'hspg' ),
					'type'     => 'password',
					'priority' => 80,
					'default'  => '',
				),
		...
		public static function get_api_credentials( $test_mode = null ) {
			if ( is_null( $test_mode ) ) {
				$test_mode = hs_get_option( 'test_mode' );
			}
			...
			return apply_filters(
				'hs_paypal_api_credentials',
				array(
					'username'  => hs_get_option( array( 'gateways_paypal', $mode . 'api_username' ) ),
					'password'  => hs_get_option( array( 'gateways_paypal', $mode . 'api_password' ) ),
					'signature' => hs_get_option( array( 'gateways_paypal', $mode . 'api_signature' ) ),
				)
			);
		}
		...
		public static function validate_donation( $valid, $gateway, $values ) {
			...
			if ( ! isset( $email ) || empty( $email ) ) {
				...
				hs_get_notices()->add_error( __( 'Missing PayPal email address. Unable to proceed with payment.', 'hspg' ) );
				return false;
		 ...
		 * @param  Hs_Donation_Processor $processor   The Donation Processor object.
		 * @return array
		 */
		public static function process_donation( $return, $donation_id, $processor ) {
			$gateway          = new Hs_Gateway_Paypal();
			 ...
			 * @param Hs_Donation_Processor $processor   The Donation Processor object.
			 */
			$paypal_args = apply_filters( 'hs_paypal_redirect_args', array(
				...
				'no_shipping'   => '1',
				'shipping'      => '0',
				'currency_code' => hs_get_currency(),
				'charset'       => get_bloginfo( 'charset' ),
				'custom'        => $donation_id,
				'rm'            => '2',
				'return'        => hs_get_permalink( 'donation_receipt_page', array( 'donation_id' => $donation_id ) ),
				'cancel_return' => hs_get_permalink( 'donation_cancel_page', array( 'donation_id' => $donation_id ) ),
				'notify_url'    => hs_get_ipn_url( Hs_Gateway_Paypal::ID ),
				'bn'            => 'Hs_SP',
		...
		public static function process_ipn() {
			...
			if ( empty( $data ) ) {
				die( __( 'Empty Data', 'hspg' ) );
			}

			if ( ! $gateway->paypal_ipn_verification( $data ) ) {
				die( __( 'IPN Verification Failure', 'hspg' ) );
			}
			...
			if ( ! $donation_id ) {
				die( __( 'Missing Donation ID', 'hspg' ) );
			}
			 ...
			 * 'hs_paypal_{transaction_type}' hook.
			 *
			 * @see Hs_Gateway_Paypal::process_web_accept()
			 */
			$txn_type = strlen( $data['txn_type'] ) ? $data['txn_type'] : 'web_accept';

			if ( has_action( 'hs_paypal_' . $txn_type ) ) {
				do_action( 'hs_paypal_' . $txn_type, $data, $donation_id );
			} else {
				do_action( 'hs_paypal_web_accept', $data, $donation_id );
			}
		...
		public static function process_web_accept( $data, $donation_id ) {
			...
			if ( array_key_exists( 'invoice', $data ) ) {
				...
				die( __( 'Missing Donation Key', 'hspg' ) );
			}
			...
			if ( strcasecmp( $business_email, trim( self::get_paypal_email() ) ) != 0 ) {
				...
				$message = sprintf( '%s %s', __( 'Invalid Business email in the IPN response. IPN data:', 'hspg' ), json_encode( $data ) );
				$donation->log()->add( $message );
				$donation->update_status( 'hs-failed' );
				die( __( 'Incorrect Business Email', 'hspg' ) );

			}

			/* Verify that the currency matches. */
			if ( hs_get_currency() != $currency_code ) {

				$message = sprintf( '%s %s', __( 'The currency in the IPN response does not match the site currency. IPN data:', 'hspg' ), json_encode( $data ) );
				$donation->log()->add( $message );
				$donation->update_status( 'hs-failed' );

				die( __( 'Incorrect Currency', 'hspg' ) );
			...
			if ( in_array( $payment_status, array( 'refunded', 'reversed' ) ) ) {
				...
				if ( $amount < $donation->get_total_donation_amount( true ) ) {
					$message = sprintf( '%s: #%s',
						__( 'Partial PayPal refund processed', 'hspg' ),
						isset( $data['parent_txn_id'] ) ? $data['parent_txn_id'] : ''
					);
				} else {
					$message = sprintf( '%s #%s %s: %s',
						__( 'PayPal Payment', 'hspg' ),
						isset( $data['parent_txn_id'] ) ? $data['parent_txn_id'] : '',
						__( 'refunded with reason', 'hspg' ),
			...
			if ( in_array( $payment_status, array( 'declined', 'failed', 'denied', 'expired', 'voided' ) ) ) {
				...
				$message = sprintf( '%s: %s', __( 'The donation has failed with the following status', 'hspg' ), $payment_status );
				$donation->log()->add( $message );
				$donation->update_status( 'hs-failed' );

				die( __( 'Payment Failed', 'hspg' ) );

			}

			/* If we have already processed this donation, stop here. */
			if ( 'hs-completed' == get_post_status( $donation_id ) ) {
				die( __( 'Donation Processed Already', 'hspg' ) );
			}

			/* Verify that the donation key matches the one stored for the donation. */
			if ( $donation_key != $donation->get_donation_key() ) {

				$message = sprintf( '%s %s', __( 'Donation key in the IPN response does not match the donation. IPN data:', 'hspg' ), json_encode( $data ) );
				$donation->log()->add( $message );
				$donation->update_status( 'hs-failed' );

				die( __( 'Invalid Donation Key', 'hspg' ) );
			...
			if ( $amount < $donation->get_total_donation_amount( true ) ) {
				...
				$message = sprintf( '%s %s', __( 'The amount in the IPN response does not match the expected donation amount. IPN data:', 'hspg' ), json_encode( $data ) );
				$donation->log()->add( $message );
				$donation->update_status( 'hs-failed' );

				die( __( 'Incorrect Amount', 'hspg' ) );
			...
			if ( 'completed' == $payment_status ) {
				...
				$message = sprintf( '%s: %s', __( 'PayPal Transaction ID', 'hspg' ), $data['txn_id'] );
				$donation->log()->add( $message );
				$donation->update_status( 'hs-completed' );

				die( __( 'Donation Completed', 'hspg' ) );
			...
			if ( 'pending' == $payment_status ) {
				...
				$donation->update_status( 'hs-pending' );

				die( __( 'Donation Pending', 'hspg' ) );

			}

			die( __( 'Unknown Response', 'hspg' ) );
		}
		...
		public function paypal_ipn_verification( $data ) {
			 ...
			 */
			return apply_filters( 'hs_paypal_ipn_verification', $this->is_valid_api_response( $api_response ), $api_response );
		}
		...
		public function get_pending_reason_note( $reason_code ) {
			switch ( $reason_code ) {
				case 'echeck':
					$note = __( 'Payment made via eCheck and will clear automatically in 5-8 days', 'hspg' );
					break;

				case 'address':
					$note = __( 'Payment requires a confirmed customer address and must be accepted manually through PayPal', 'hspg' );
					break;

				case 'intl':
					$note = __( 'Payment must be accepted manually through PayPal due to international account regulations', 'hspg' );
					break;

				case 'multi-currency':
					$note = __( 'Payment received in non-shop currency and must be accepted manually through PayPal', 'hspg' );
					break;

				case 'paymentreview':
				case 'regulatory_review':
					$note = __( 'Payment is being reviewed by PayPal staff as high-risk or in possible violation of government regulations', 'hspg' );
					break;

				case 'unilateral':
					$note = __( 'Payment was sent to non-confirmed or non-registered email address.', 'hspg' );
					break;

				case 'upgrade':
					$note = __( 'PayPal account must be upgraded before this payment can be accepted', 'hspg' );
					break;

				case 'verify':
					$note = __( 'PayPal account is not verified. Verify account in order to accept this payment', 'hspg' );
					break;

				default:
					/* translators: %s: reason code given by PayPal */
					$note = sprintf( __( 'Payment is pending for unknown reasons. Contact PayPal support for assistance. Reason code: %s', 'hspg' ), $reason_code );
			}
			 ...
			 * @param string $reason_code The reason code received from PayPal.
			 */
			return apply_filters( 'hs_paypal_gateway_pending_reason_note', $note, $reason_code );
		}
		...
		public static function process_refund( $donation_id ) {
			...
			foreach ( $donation->get_campaign_donations() as $campaign_donation ) {
				...
				$body[ 'L_DESCRIPTION' . $n ]       = __( 'Donation', 'hspg' );
				$body[ 'L_PRICE' . $n ]             = Hs_Currency::get_instance()->sanitize_monetary_amount( (string) $campaign_donation->amount );
				$body[ 'L_PRICECURRENCYCODE' . $n ] = hs_get_currency();
			 ...
			 * @param Hs_Donation $donation The donation instance.
			 */
			$body    = apply_filters( 'hs_paypal_refund_args', $body, $donation );
			$headers = array(
				'Content-Type'  => 'application/x-www-form-urlencoded',
			...
			if ( is_wp_error( $request ) ) {
				$donation->log()->add( sprintf(
					...
					__( 'PayPal refund failed: %s', 'hspg' ),
			...
			if ( 200 === (int) $code
				...
				$donation->log()->add( sprintf(
					...
					__( 'PayPal refund transaction ID: %s', 'hspg' ),
			...
			$error = array_key_exists( 'L_LONGMESSAGE0', $body ) ? $body['L_LONGMESSAGE0'] : __( 'Unknown reason', 'hspg' );
		 ...
		 * @return boolean
		 */
		public function is_donation_refundable( Hs_Donation $donation ) {
		...
		public function get_redirect_url( $ssl_check = false, $ipn_check = false ) {
			 ...
			 * @param boolean $ipn_check Whether this is for an IPN request.
			 */
			return apply_filters( 'hs_paypal_uri', $paypal_uri, $ssl_check, $ipn_check );
		}
		...
		public static function get_paypal_email() {
			$option = hs_get_option( 'test_mode' ) ? 'sandbox_paypal_email' : 'paypal_email';
			 ...
			 */
			return apply_filters( 'hs_paypal_email', hs_get_option( array( 'gateways_paypal', $option ) ) );
		}
