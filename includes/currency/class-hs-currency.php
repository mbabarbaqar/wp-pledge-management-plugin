<?php
/**
 * Hspg Currency helper.
 *
 * @package   Hspg/Classes/Hs_Currency
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Currency' ) ) :
	 ...
	 * @since 1.0.0
	 */
	final class Hs_Currency {

		/**
		 * The single instance of this class.
		 *
		 * @var Hs_Currency|null
		 */
		...
		private function __construct() {
			add_filter( 'hs_option_decimal_count', array( $this, 'maybe_force_zero_decimals' ) );
		}
		...
		public function get_monetary_amount( $amount, $decimal_count = false, $db_format = false, $symbol_position = '' ) {
			if ( false === $decimal_count ) {
				$decimal_count = hs_get_option( 'decimal_count', 2 );
			}
			 ...
			 */
			return apply_filters( 'hs_monetary_amount', $formatted, $amount );
		}
		...
		public function sanitize_monetary_amount( $amount, $db_format = false ) {
			...
			if ( ! is_string( $amount ) ) {
				...
				hs_get_deprecated()->doing_it_wrong(
					__METHOD__,
					__( 'Amount must be passed as a string.', 'hspg' ),
					'1.0.0'
				);
		 ...
		 *
		 * This effectively reverses the effect of Hs_Currency::sanitize_database_amount.
		 *
		 * @since  1.6.0
		 *
		...
		public function get_currency_format( $symbol_position = '' ) {
			if ( empty( $symbol_position ) ) {
				$symbol_position = hs_get_option( 'currency_format', 'left' );
			}
			...
			switch ( $symbol_position ) {
				...
				default:
					 ...
					 */
					$format = apply_filters( 'hs_currency_format', '%1$s%2$s', $symbol_position );
			}
		...
		public function get_accounting_js_format() {
			switch ( hs_get_option( 'currency_format', 'left' ) ) {
				case 'right':
					$format = '%v%s';
					break;
				case 'left-with-space':
			 ...
			 * @param string $format The currency format.
			 */
			return apply_filters( 'hs_accounting_js_currency_format', $format );
		}
		...
		public function get_all_currencies() {
			if ( empty( $this->currencies ) ) {
				/**
				 * Filter the currencies available in Hspg.
				 *
				 * @since 1.0.0
				 *
				...
				$this->currencies = apply_filters(
					...
					array(
						...
						'TOP' => sprintf(
							/* translators: %s: currency symbol */
							__( 'Tongan Pa&lsquo;anga (%s)', 'hspg' ),
							$this->get_currency_symbol( 'TOP' )
						),
						'TRY' => sprintf(
							/* translators: %s: currency symbol */
							__( 'Turkish Lira (%s)', 'hspg' ),
						...
						'VUV' => sprintf(
							/* translators: %s: currency symbol */
							__( 'Vanuatu Vatu (%s)', 'hspg' ),
							$this->get_currency_symbol( 'VUV' )
						),
						'VND' => sprintf(
							/* translators: %s: currency symbol */
							__( 'Vietnamese Dong (%s)', 'hspg' ),
		...
		public function get_currency_symbol( $currency = '' ) {
			...
			return apply_filters( 'hs_currency_symbol', $currency_symbol, $currency );
		}
		...
		public function get_thousands_separator() {
			$separator = hs_get_option( 'thousands_separator', ',' );
		...
		public function get_decimal_separator() {
			return hs_get_option( 'decimal_separator', '.' );
		}
		...
		public function get_decimals() {
			...
			return hs_get_option( 'decimal_count', $default );
		}
