 ...
 *
 * @package   Hspg/Shortcodes/Campaigns
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Campaigns_Shortcode' ) ) :

	/**
	 * Hs_Campaigns_Shortcode class.
	 *
	 * @since   1.0.0
	 */
	class Hs_Campaigns_Shortcode {
		...
		public static function display( $atts ) {
			 ...
			 */
			$template = apply_filters( 'hs_campaigns_shortcode_template', false, $args );
			...
			if ( ! is_object( $template ) || ! is_a( $template, 'Hs_Template' ) ) {
				$template = new Hs_Template( 'campaign-loop.php', false );
			}
			 ...
			 * @param array $args      All the parsed arguments.
			 */
			$view_args = apply_filters( 'hs_campaigns_shortcode_view_args', hs_array_subset( $args, array( 'campaigns', 'columns', 'button', 'responsive', 'masonry' ) ), $args );
			 ...
			 * @return string
			 */
			return apply_filters( 'hs_campaigns_shortcode', ob_get_clean(), $args );
		}
		...
		public static function get_campaigns( $args ) {
			 ...
			 */
			$query_args = apply_filters( 'hs_campaigns_shortcode_query_args', $query_args, $args );
			...
			switch ( $args['orderby'] ) {
				case 'popular':
					return Hs_Campaigns::ordered_by_amount( $query_args );

				case 'ending':
					return Hs_Campaigns::ordered_by_ending_soon( $query_args );

				default:
					return Hs_Campaigns::query( $query_args );
			}
		}
	}
