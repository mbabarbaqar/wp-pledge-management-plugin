 ...
 *
 * @package   Hspg/Classes/Hs_Endpoints
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Endpoints' ) ) :
	...
	class Hs_Endpoints {
		...
		public function get_page_url( $endpoint, $args = array() ) {
			 ...
			 *
			 * The hook takes the format of hs_permalink_{endpoint}_page. For example,
			 * for the campaign_donation endpoint, the hook is:
			 *
			 * hs_permalink_campaign_donation_page
			 *
			 * @since 1.0.0
			 *
			 ...
			 */
			return apply_filters( 'hs_permalink_' . $endpoint . '_page', $default, $args );
		}
		...
		public function is_page( $endpoint, $args = array() ) {
			 ...
			 *
			 * The hook takes the format of hs_is_page_{endpoint}_page. For example,
			 * for the campaign_donation endpoint, the hook is:
			 *
			 * hs_is_page_campaign_donation_page
			 *
			 * @since 1.0.0
			 *
			 ...
			 * @param array   $args    Mixed set of arguments.
			 */
			return apply_filters( 'hs_is_page_' . $endpoint . '_page', $default, $args );
		}
			...
			if ( ! $this->endpoint_exists( $endpoint ) ) {
				hs_get_deprecated()->doing_it_wrong(
					...
					sprintf(
						...
						__( 'Endpoint %s has not been registered.', 'hspg' ), $endpoint
					),
					'1.5.0'
				);
		...
		public function disable_endpoint_cache() {
			 ...
			 * @since 1.6.14
			 */
			do_action( 'hs_do_not_cache' );
		}

		/**
		 * If we're viewing a Hspg endpoint, we're not on a 404, even
		...
		public function prevent_polylang_rewrite_modification( $modify, $rule ) {
			...
			$protected_rules = apply_filters(
				'hs_polylang_protected_rewrite_rules',
				array(
					'hs-listener(/(.*))?/?$',
				)
			);
		...
		public function template_loader( $template ) {
			...
			$template_options = apply_filters( 'hs_' . $current_endpoint . '_page_template', $template_options );

			return hs_get_template_path( $template_options, $template );
		}
		...
		public function maybe_remove_comments_template( $template ) {
			...
			if ( $endpoint && $this->endpoints[ $endpoint ]->comments_disabled() ) {
				$template = hs_get_template_path( 'comments/disabled-comments.php' );
			}
		...
		public function endpoints_menu_meta_box() {
			...
			$walker = new Hs_Walker_Nav_Menu_Checklist();
			...
			?>
			<div id="hspg" class="categorydiv">
				<ul id="hs-tabs" class="hs-tabs add-menu-item-tabs">
					<li <?php echo ( 'all' == $current_tab ? ' class="tabs"' : '' ); ?>>
						<a class="nav-tab-link" data-type="tabs-panel-hs-all" href="<?php if ( $nav_menu_selected_id ) echo esc_url( add_query_arg( 'hs-tab', 'all', remove_query_arg( $removed_args ) ) ); ?>#tabs-panel-hs-all">
							<?php _e( 'View All', 'hspg' ); ?>
						</a>
					</li><!-- /.tabs -->
				</ul>
				<div id="tabs-panel-hs-all" class="tabs-panel tabs-panel-view-all <?php echo ( 'all' == $current_tab ? 'tabs-panel-active' : 'tabs-panel-inactive' ); ?>">
					<ul id="hs-checklist-all" class="categorychecklist form-no-clear">
					<?php
				...
				<p class="button-controls wp-clearfix">
					<span class="list-controls">
						<a href="<?php echo esc_url( add_query_arg( array( 'hs-tab' => 'all', 'selectall' => 1, ), remove_query_arg( $removed_args ) ) ); ?>#hspg" class="select-all"><?php _e( 'Select All', 'hspg' ); ?></a>
					</span>
					<span class="add-to-menu">
						<input type="submit"<?php wp_nav_menu_disabled_check( $nav_menu_selected_id ); ?> class="button-secondary submit-add-to-menu right" value="<?php esc_attr_e( 'Add to Menu', 'hspg' ); ?>" name="add-hs-menu-item" id="submit-hs" />
		...
		/**
		 * Add Hspg menu meta box to the Customizer.
		 *
		 * @since  1.6.29
		 *
		...
		public function add_endpoints_menu_meta_box_to_customizer( $item_types ) {
			return array_merge(
				...
				array(
					'hs_nav' => array(
						'title'  => _x( 'Hspg', 'customizer menu section title', 'hspg' ),
						'type'   => 'hs_nav',
						'object' => 'hs_nav',
					),
				)
		 ...
		 * @return array The paginated Hspg user nav items.
		 */
		public function add_endpoints_menu_meta_box_items_to_customizer( $items = array(), $type = '', $object = '', $page = 0 ) {
			...
			foreach ( $this->get_endpoints_for_nav_menu() as $item ) {
				$item               = (array) $item;
				$item['id']         = 'hs-' . $item['object_id'];
				$item['classes']    = implode( ' ', $item['classes'] );
				$item['type_label'] = _x( 'Custom Link', 'customizer menu type label', 'hspg' );
					...
					foreach ( $endpoint_ids as $endpoint_id ) {
						...
						if ( ! isset( $this->endpoints[ $endpoint_id ] ) ) {
							error_log(
								sprintf(
									/* translators: %s: endpoint id */
									__( 'Endpoint %s was incorrectly registered.', 'hspg' ),
									$endpoint_id
								)
							);
		 ...
		 *
		 * @param  string $endpoint The endpoint ID.
		 * @return Hs_Endpoint|false False if no endpoint exists, or the object.
		 */
