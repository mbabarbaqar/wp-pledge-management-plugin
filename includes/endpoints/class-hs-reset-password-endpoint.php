 ...
 *
 * @package   Hspg/Classes/Hs_Reset_Password_Endpoint
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Reset_Password_Endpoint' ) ) :

	/**
	 * Hs_Reset_Password_Endpoint
	 *
	 * @since 1.5.0
	 */
	class Hs_Reset_Password_Endpoint extends Hs_Endpoint {
		...
		public function get_page_url( $args = array() ) {
			...
			if ( wp_login_url() == $login_page ) {
				hs_get_deprecated()->doing_it_wrong(
					__FUNCTION__,
					__( 'Password reset link should not be called when using the default WordPress login.', 'hspg' ),
					'1.4.0'
				);
		...
		public function get_template( $template ) {
			...
			new Hs_Ghost_Page(
				'reset-password-page',
				array(
					'title'   => __( 'Reset Password', 'hspg' ),
					'content' => '<!-- Silence is golden -->',
				)
			);

			return hs_splice_template( get_page_template_slug( $login ), array( 'reset-password-page.php', 'page.php', 'singular.php', 'index.php' ) );
		}
