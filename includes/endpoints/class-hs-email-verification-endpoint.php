 ...
 *
 * @package   Hspg/Classes/Hs_Email_Verification_Endpoint
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Email_Verification_Endpoint' ) ) :

	/**
	 * Hs_Email_Verification_Endpoint
	 *
	 * @since 1.5.0
	 */
	class Hs_Email_Verification_Endpoint extends Hs_Endpoint {
		...
		public function get_redirect() {
			...
			if ( is_a( $result, 'WP_User' ) ) {
				hs_get_notices()->add_success( __( 'Your email address has been verified.', 'hspg' ) );
		...
		public function get_template( $template ) {
			...
			new Hs_Ghost_Page(
				...
				array(
					'title'   => __( 'Email Verification', 'hspg' ),
			...
			if ( $profile ) {
				return hs_splice_template( get_page_template_slug( $profile ), $templates );
			}
		...
		protected function get_verification_check_result() {
			if ( ! isset( $this->verification_result ) ) {
				...
				if ( is_a( $this->verification_result, 'WP_User' ) ) {
					hs_get_user( get_user_by( 'login', $_GET['login'] ) )->mark_as_verified( true );
				}
			}
		...
		protected function get_base_page() {
			$profile = hs_get_permalink( 'profile' );
