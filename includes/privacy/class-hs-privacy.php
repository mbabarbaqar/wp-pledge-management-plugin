<?php
/**
 * A class providing privacy tools for Hspg.
 *
 * @package   Hspg/Classes/Hs_Privacy
 * @author    Eric Daams
	...
	class Hs_Privacy {
		...
		public function get_data_retention_fields() {
			if ( ! isset( $this->data_retention_fields ) ) {
				$this->data_retention_fields = hs_get_option( 'data_retention_fields', array_keys( $this->get_user_donation_fields() ) );
			}
		...
		public function get_data_retention_period() {
			return hs_get_option( 'minimum_data_retention_period', 2 );
		}
		 ...
		 * @since  1.6.0
		 *
		 * @return Hs_Donation_Field[]
		 */
		public function get_user_donation_fields() {
			if ( ! isset( $this->user_donation_fields ) ) {
				$this->user_donation_fields = hspg()->donation_fields()->get_data_type_fields( 'user' );
			}
		...
		public function register_exporter( $exporters ) {
			return array_merge( $exporters, array(
				array(
					'exporter_friendly_name' => __( 'Hspg Donor Data', 'hspg' ),
		...
		public function register_eraser( $erasers ) {
			return array_merge( $erasers, array(
				array(
					'eraser_friendly_name' => __( 'Hspg Donor Data Eraser', 'hspg' ),
		...
		/**
		 * Add privacy policy content for Hspg.
		 *
		 * @since  1.6.0
		 *
		 * @return void
		...
		public function add_privacy_policy_content() {
			...
			/**
			 * Filter the Hspg privacy policy content.
			 *
			 * @since 1.6.0
			 *
			 * @param string $content The privacy policy content.
			 */
			$content = wp_kses_post( apply_filters( 'hs_privacy_policy_content', wpautop( __( '
...
', 'hspg' ) ) ) );
			...
			wp_add_privacy_policy_content( 'Hspg', $content );
		}
		 ...
		 * 2. Donor profile records. (*_hs_donors table)
		...
		public function export_user_data( $email, $page = 1 ) {
			...
			/* 2. Get donor profile. */
			$profiles = hs_get_table( 'donors' )->get_personal_data( $email );
			...
			if ( ! empty( $user_donation_fields ) ) {
				...
				$donations    = hs_get_table( 'campaign_donations' )->get_distinct_ids( 'donation_id', $donor_id, 'donor_id' );
				$export_items = array_merge(
					$export_items,
		...
		public function erase_user_data( $email, $page = 1 ) {
			...
			if ( is_array( $profiles ) ) {
				hs_get_table( 'donors' )->erase_donor_data( wp_list_pluck( $profiles, 'donor_id' ) );
		...
		public function get_registered_donor_data( $user ) {
			...
			$key_map = hs_get_user_mapped_keys();
			 ...
			 * @param WP_User $user An instance of `WP_User`.
			 */
			$data = apply_filters( 'hs_privacy_export_personal_registered_donor_data', $data, $user );

			return array(
				'item_id'     => 'user',
				'group_id'    => 'hs_users',
				'group_label' => __( 'Donor Meta', 'hspg' ),
				'data'        => $data,
			);
		}
		...
		protected function get_donor_profile_data( $profile ) {
			 ...
			 * @param object $profile The profile record.
			 */
			$data = apply_filters( 'hs_privacy_export_personal_donor_profile_data', array(
				array(
					'name'  => __( 'First Name', 'hspg' ),
					'value' => $profile->first_name,
				),
				array(
					'name'  => __( 'Last Name', 'hspg' ),
			...
			return array(
				...
				'group_id'    => 'hs_donors',
				'group_label' => __( 'Donor Profiles', 'hspg' ),
				'data'        => $data,
			);
		}
		...
		protected function get_personal_donation_meta_data( $donation_id ) {
			 ...
			 * @param int   $donation_id The donation ID.
			 */
			$data = apply_filters( 'hs_privacy_export_personal_donation_meta_data', $data, $meta, $donation_id );

			return array(
				'item_id'     => 'donation-' . $donation_id,
				'group_id'    => 'hs_donations',
				'group_label' => __( 'Donation Meta', 'hspg' ),
				'data'        => $data,
			);
		}
		...
		protected function erase_registered_donor_personal_data( WP_User $user ) {
			...
			foreach ( $methods as $method ) {
				...
				foreach ( $fields as $key => $field ) {
					 ...
					 */
					$data = apply_filters( 'hs_privacy_erasure_registered_user_data_prop_value', wp_privacy_anonymize_data( $data_type, $user->{$key} ), $user->{$key}, $key, $field );
		...
		protected function erase_donation_personal_data( $donation_id ) {
			...
			foreach ( $this->get_erasable_fields_for_donation( $donation_id ) as $field_id => $field ) {
				 ...
				 * @param mixed                     $value The current value of the field.
				 * @param string                    $key   The key of the field.
				 * @param Hs_Donation_Field $field The field definition.
				 */
				$meta[ $field_id ] = apply_filters( 'hs_privacy_erasure_donation_donor_data_prop_value', $replace, $meta[ $field_id ], $field_id, $field );
			}
			...
			wp_update_post( array(
				...
				'post_title' => sprintf(
					...
					__( 'Anonymous &ndash; %s', 'hspg' ),
					hs_get_donation( $donation_id )->get_campaigns_donated_to()
				),
			) );

			$log = new Hs_Donation_Log( $donation_id );
			$log->add( __( 'Personal data erased.', 'hspg' ) );
		}
		...
		protected function get_donations_from_profiles( $profiles ) {
			...
			return hs_get_table( 'campaign_donations' )->get_distinct_ids( 'donation_id', $donor_id, 'donor_id' );
		}
		 ...
		 * @since  1.6.0
		 *
		 * @param  object $field The field attributes. This may be a Hs_Donation_Field
		...
		protected function get_field_data_type( $field ) {
			 ...
			 *                          instance, or a simple object cast from an array.
			 */
			return apply_filters( 'hs_privacy_erasure_field_data_type', $data_type, $field );
		}
	}
