<?php
/**
 * Contains the class that models users in Hspg.
 *
 * There are several different user roles in Hspg, and one user
 * may be more than one. People who make donations get the "donor" role;
 * people who create campaigns (via Hspg Ambassadors) get
 * the "campaign_creator" role; people who create fundraisers for campaigns
 * get the "fundraiser" role.
 *
 * @package   Hspg/Classes/Hs_User
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_User' ) ) :
	...
	class Hs_User extends WP_User {
		...
		public static function init_with_donor( $donor_id ) {
			$user_id = hs_get_table( 'donors' )->get_user_id( $donor_id );
		...
		public function get_donor() {
			...
			if ( isset( $this->donor_id ) ) {
				...
				$donor = hs_get_table( 'donors' )->get( $this->donor_id );

			} else {

				$donor = hs_get_table( 'donors' )->get_by( 'email', $this->get( 'user_email' ) );
		...
		public function is_verified() {
			return absint( get_user_meta( $this->ID, '_hs_user_verified', true ) );
		}
		...
		public function mark_as_verified( $verified ) {
			$verified = update_user_meta( $this->ID, '_hs_user_verified', $verified );
			...
			delete_user_meta( $this->ID, '_hs_email_verification_email_send_time' );

			/* Check for a notice about having sent a verification email and remove it. */
			hs_get_notices()->clear_notice( "/We have sent you an email to confirm your email address. Haven\'t received the email\?/" );
			 ...
			 * @since 1.6.23
			 *
			 * @param Hs_User $user The user object.
			 */
			do_action( 'hs_user_verified', $this );
		...
		public function get_email() {
			 ...
			 * @param Hs_User $user  This instance of `Hs_User`.
			 */
			return apply_filters( 'hs_user_email', $email, $this );
		}
		...
		public function get_name() {
			...
			return apply_filters( 'hs_user_name', $name, $this );
		}
		...
		public function get_first_name() {
			...
			return apply_filters( 'hs_user_first_name', $name, $this );
		}
		...
		public function get_last_name() {
			...
			return apply_filters( 'hs_user_last_name', $name, $this );
		}
		...
		public function get_location() {
			...
			return apply_filters( 'hs_user_location', $location, $this );
		}
		...
		public function get_address( $donation_id = '' ) {
			 ...
			 *
			 * Please note that prior to 1.6.44, the hook used here was `hs_user_address_fields`,
			 ...
			 */
			$address_details = apply_filters( 'hs_user_address_details', $address_details, $this, $donation_id );

			return hs_get_location_helper()->get_formatted_address( $address_details );
		}
		...
		public function get_donations( $distinct_donations = false ) {
			return hs_get_table( 'campaign_donations' )->get_donations_by_donor( $this->get_donor_id(), $distinct_donations );
		}
		...
		public function count_donations( $distinct_donations = false ) {
			return hs_get_table( 'campaign_donations' )->count_donations_by_donor( $this->get_donor_id(), $distinct_donations );
		}
		...
		public function count_campaigns_supported() {
			return hs_get_table( 'campaign_donations' )->count_campaigns_supported_by_donor( $this->get_donor_id() );
		}
		...
		public function get_total_donated( $campaign_id = false ) {
			$amount = wp_cache_get( $this->get_donor_id(), 'hs_donor_total_donation_amount_' . $campaign_id );

			if ( false === $amount ) {

				$args = apply_filters( 'hs_user_total_donated_query_args', array(
					'output'          => 'raw',
					'donor_id'        => $this->get_donor_id(),
				...
				$query = new Hs_Donor_Query( $args );

				$amount = $query->current()->amount;

				wp_cache_set( $this->get_donor_id(), $amount, 'hs_donor_total_donation_amount_' . $campaign_id );
			}
		...
		public function get_avatar( $size = 100 ) {
			...
			$avatar_attachment_id = apply_filters( 'hs_user_avatar', false, $this );
			...
			$img_size = apply_filters( 'hs_user_avatar_media_size', array( $size, $size ), $size );
			...
			return apply_filters( 'hs_user_avatar_custom', sprintf( '<img src="%s" alt="%s" class="avatar photo" width="%s" height="%s" />',
				$attachment_src[0],
		...
		public function get_avatar_src( $size = 100 ) {
			...
			$avatar = apply_filters( 'hs_user_avatar_src', false, $this, $size );
		...
		public function get_activity( $args = array() ) {
			$defaults = array(
				'author'      => $this->ID,
				'post_status' => array( 'hs-completed', 'hs-preapproved', 'publish' ),
			...
			$args = apply_filters( 'hs_user_activity_args', $args, $this );
		...
		public function add_donor( $submitted = array() ) {
			 ...
			 *
			 * Prior to Hspg 1.6, this was never permitted. As of Hspg 1.6, donors
			 ...
			 */
			if ( ! $email && ! hs_permit_donor_without_email() ) {
				hs_get_deprecated()->doing_it_wrong(
					__METHOD__,
					__( 'Unable to add donor. Email not set for logged out user.', 'hspg' ),
					'1.0.0'
				);
			...
			$donor_id = hs_get_table( 'donors' )->insert( $donor_values );
			...
			do_action( 'hs_after_insert_donor', $donor_id, $this );
		...
		public function update_donor_record( $submitted, $keys ) {
			$donor_fields = array_intersect( $keys, hs_get_donor_keys() );
			...
			if ( $donor_id ) {
				hs_get_table( 'donors' )->update( $donor_id, $donor_data );

				return $donor_id;
			}

			return hs_get_table( 'donors' )->insert( $donor_data );
		}
		...
		public function update_core_user( $submitted ) {
			...
			if ( 0 == $this->ID ) {
				...
				if ( ! isset( $values['user_pass'] ) || strlen( $values['user_pass'] ) == 0 ) {
					hs_get_notices()->add_error( '<strong>ERROR:</strong> Password field is required.' );
					return false;
				}
				...
				if ( is_wp_error( $user_id ) ) {
					hs_get_notices()->add_errors_from_wp_error( $user_id );
					return false;
				}

				$this->init( self::get_data_by( 'id', $user_id ) );

				$signon = Hs_User::signon( $values['user_login'], $values['user_pass'] );

				if ( is_wp_error( $signon ) ) {
					hs_get_notices()->add_errors_from_wp_error( $signon );
					return false;
				}
				 ...
				 */
				do_action( 'hs_after_insert_user', $user_id, $values );
			 ...
			 */
			do_action( 'hs_after_save_user', $user_id, $values );
		...
		public static function signon( $username, $password ) {
			if ( ! apply_filters( 'hs_auto_login_after_registration', true, $username ) ) {
				return false;
			}
		...
		public function get_mapped_keys() {
			if ( ! isset( $this->mapped_keys ) ) {
				$this->mapped_keys = hs_get_user_mapped_keys();
			}
		...
		public function get_core_keys() {
			if ( ! isset( $this->core_keys ) ) {
				$this->core_keys = hs_get_user_core_keys();
			}
		...
		public function get_address_fields() {
			...
			return apply_filters(
				'hs_user_address_fields_deprecated',
				array(
					'donor_address',
					'donor_address_2',
					'donor_city',
