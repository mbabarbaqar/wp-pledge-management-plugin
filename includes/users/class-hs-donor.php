...
if ( ! class_exists( 'Hs_Donor' ) ) :
	...
	class Hs_Donor {
		...
		/**
		 * The donor data from hs_donors table.
		 *
		 * @since 1.0.0
		 *
		 * @var   Object
		...
		public function __construct( $donor_id, $donation_id = false ) {
			...
			$this->data        = hs_get_table( 'donors' )->get( $donor_id );
			$this->donation_id = $donation_id;
		}

		/**
		 * Magic getter method. Looks for the specified key in as a property before using Hs_User's __get method.
		 *
		 * @since  1.0.0
		 *
		...
		/**
		 * A thin wrapper around the Hs_User::get() method.
		 *
		 * @since  1.2.4
		 *
		...
		/**
		 * Return the Hs_User object for this donor.
		 *
		 * @since  1.0.0
		 *
		 * @return Hs_User
		...
		public function get_user() {
			if ( ! isset( $this->user ) ) {
				$this->user = Hs_User::init_with_donor( $this->donor_id );
			}
		...
		/**
		 * Return the Hs_Donation object associated with this object.
		 *
		 * @since  1.0.0
		 *
		...
		public function get_donation() {
			if ( ! isset( $this->donation ) ) {
				$this->donation = $this->donation_id ? hs_get_donation( $this->donation_id ) : false;
			}
		...
		public function set_user_id( $user_id ) {
			return hs_get_table( 'donors' )->update( $this->donor_id, array( 'user_id' => $user_id ), 'donor_id' );
		}
		 ...
		 * @since  1.5.7
		 *
		 * @return Hs_Donation|null Null if no donation was found. A `Hs_Donation` instance otherwise.
		 */
		public function get_last_donation() {
			if ( ! isset( $this->last_donation ) ) {
				$donation = new Hs_Donations_Query(
					array(
						'number'   => 1,
		...
		public function get_name() {
			 ...
			 * @param Hs_Donor $donor This instance of `Hs_Donor`.
			 */
			return apply_filters( 'hs_donor_name', $name, $this );
		}
		...
		public function has_valid_email() {
			return hs_is_valid_email_address( $this->get_donor_meta( 'email' ) );
		}
		...
		public function get_avatar( $size = 100 ) {
			 ...
			 * @param Hs_Donor $donor  This instance of `Hs_Donor`.
			 */
			return apply_filters( 'hs_donor_avatar', $this->get_user()->get_avatar( $size ), $this );
		}
		...
		public function get_location() {
			...
			return apply_filters( 'hs_donor_location', $location, $this );
		}
		...
		public function get_donation_amount( $campaign_id = '' ) {
			return apply_filters( 'hs_donor_donation_amount', hs_get_table( 'campaign_donations' )->get_donation_amount( $this->donation_id, $campaign_id ), $this, $campaign_id );
		}
