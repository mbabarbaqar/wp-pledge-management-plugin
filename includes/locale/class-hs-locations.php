 ...
 *
 * @package   Hspg/Classes/Hs_Locations
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Locations' ) ) :
	...
	class Hs_Locations {
		...
		/**
		 * Return an array with all the countries supported by Hspg.
		 *
		 * @since  1.0.0
		 *
		 * @return string[]
		...
		public function get_countries() {
			if ( empty( $this->countries ) ) {
				/**
				 * Filter the list of countries supported by Hspg.
				 *
				 * @since 1.0.0
				 *
				 * @param array $countries The list of countries.
				 */
				$this->countries = apply_filters( 'hs_countries', include( hspg()->get_path( 'directory' ) . 'i18n/countries.php' ) );
			}
		...
		/**
		 * Return an array with all the states supported by Hspg.
		 *
		 * @since  1.0.0
		 *
		 * @return string[]
		...
		public function get_states_for_country( $country_code ) {
			...
			if ( ! isset( $this->states[ $country_code ] ) ) {
				...
				$this->states[ $country_code ] = apply_filters(
					'hs_country_states',
					include( hspg()->get_path( 'directory' ) . '/i18n/states/' . $country_code . '.php' ),
					$country_code
				);
			}
		...
		public function get_base_country() {
			$country = esc_attr( get_option( 'hs_default_country' ) );
			 ...
			 */
			return apply_filters( 'hs_countries_base_country', $country );
		}
		...
		public function get_address_formats() {
			if ( ! isset( $this->address_formats ) ) {
				...
				$this->address_formats = apply_filters(
					'hs_localisation_address_formats',
					array(
						'default' => "{name}\n{company}\n{address_1}\n{address_2}\n{city}\n{state}\n{postcode}\n{country}",
						'AU' => "{name}\n{company}\n{address_1}\n{address_2}\n{city} {state} {postcode}\n{country}",
		...
		private function get_address_format( $address_fields ) {
			 ...
			 *
			 * Use the `hs_formatted_address_force_country_display` filter
			 ...
			 */
			if ( $country == $this->get_base_country() && false === apply_filters( 'hs_formatted_address_force_country_display', false ) ) {
		...
		public function get_formatted_address( $address_fields = array() ) {
			...
			$replacements = apply_filters(
				'hs_formatted_address_replacements',
				array(
