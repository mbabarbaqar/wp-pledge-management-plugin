<?php
/**
 * Responsible for holding instances of Hspg helper objects.
 *
 * @package   Hspg/Classes/Hs_Registry
 * @author    Eric Daams
...
if ( ! class_exists( 'Hs_Registry' ) ) :
	...
	class Hs_Registry {
		...
		protected function get_class_key( $class ) {
			if ( false === strpos( $class, 'Hs_' ) ) {
				return $class;
			}

			return strtolower( str_replace( 'Hs_', '', $class ) );
		}
		...
		protected function get_class_instance( $class ) {
			if ( ! class_exists( $class ) ) {
				...
				wp_die( sprintf( _x( 'Class %s does not exist.', 'error message when non-existent class is called', 'hspg' ), $class ) );
			}
