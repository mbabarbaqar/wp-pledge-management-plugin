...
( function( exports, $ ) {
    ...
    var Donation_Form = function( form ) {
        ...
        var on_click_terms = function() {
            self.form.find( '.hs-terms-text' ).addClass( 'active' );
            return false;
        };
        ...
        var trigger_amount_change_events = function() {
            ...
            $body.trigger( 'hspg:form:amount:changed', self );

            /* The overall donation amount has changed. */
            $body.trigger( 'hspg:form:total:changed', self );
        };
        ...
        var on_select_donation_amount = function() {
            ...
            $li.parents( '.hs-donation-form' ).find( '.donation-amount.selected' ).removeClass( 'selected' );
        ...
        var on_click_change_amount_link = function() {
            $(this).parent().addClass( 'hs-hidden' );
        };
        ...
        var on_submit = function( event ) {
            ...
            maybe_process( helper, function() {
                $body.trigger( 'hspg:form:process', helper );
            } );
         ...
         *
         * This is a callback for the 'hspg:form:process' event. It's called
        ...
        var process_donation = function( event, helper ) {
            ...
            /* Cancel the default Hspg action, but pass it along as the form_action variable */
            data.action = 'make_donation';
            data.form_action = data.hs_action;
            delete data.hs_action;
            ...
            $.ajax({
                ...
                success: function (response) {
                    $body.trigger( 'hspg:form:processed', [ response, helper ] );
        ...
        var init = function() {
            ...
            self.form.on( 'focus', 'input.custom-donation-input', on_focus_custom_amount );
            self.form.on( 'click', '.hs-terms-link', on_click_terms );
            ...
            if ( self.get_all_payment_methods().length ) {
                self.hide_inactive_payment_methods();
                self.form.on( 'change', '#hs-gateway-selector input[name=gateway]', on_change_payment_gateway );
            }
            ...
            if ( false === CHARITABLE.forms_initialized ) {
                ...
                // Process the donation on the 'hspg:form:process' event.
                $body.on( 'hspg:form:process', process_donation );

                $body.trigger( 'hspg:form:initialize', self );

                CHARITABLE.forms_initialized = true;
            }

            $body.trigger( 'hspg:form:loaded', self );
        }
    ...
    Donation_Form.prototype.get_custom_amount = function() {
        var input = this.form.find( '.hs-donation-options.active input.custom-donation-input,.hs-donation-options.active input.custom-donation-amount' );
    ...
    Donation_Form.prototype.get_amount = function() {
        ...
        this.form.trigger( 'hspg:form:get_amount', this );
    ...
    Donation_Form.prototype.get_cc_number = function() {
        return this.form.find( '#hs_field_cc_number input' ).val() || '';
    };
    ...
    Donation_Form.prototype.get_cc_cvc = function() {
        return this.form.find( '#hs_field_cc_cvc input' ).val() || '';
    };
    ...
    Donation_Form.prototype.get_cc_expiry_month = function() {
        return this.form.find( '#hs_field_cc_expiration select.month' ).val() || '';
    };
    ...
    Donation_Form.prototype.get_cc_expiry_year = function() {
        return this.form.find( '#hs_field_cc_expiration select.year' ).val() || '';
    };
    ...
    Donation_Form.prototype.clear_cc_fields = function() {
        this.form.find( '#hs_field_cc_number input, #hs_field_cc_name input, #hs_field_cc_cvc input, #hs_field_cc_expiration select' ).removeAttr( 'name' );
    };
    ...
    Donation_Form.prototype.get_all_payment_methods = function() {
        return this.form.find( '#hs-gateway-selector input[name=gateway]' );
    }
    ...
    Donation_Form.prototype.hide_inactive_payment_methods = function() {
        ...
        var fields = this.form.find( '.hs-gateway-fields[data-gateway!=' + active + ']' );
    ...
    Donation_Form.prototype.print_errors = function( errors ) {
        ...
        if ( this.form.find( '.hs-form-errors' ).length ) {
            this.form.find( '.hs-form-errors' ).remove();
        }

        output += '<div class="hs-form-errors hs-notice"><ul class="errors"><li>';
        output += e.join( '</li><li>' );
        output += '</li></ul></div>';
    ...
    Donation_Form.prototype.show_processing = function() {
        this.form.find( '.hs-form-processing' ).show();
    ...
    Donation_Form.prototype.hide_processing = function() {
        this.form.find( '.hs-form-processing' ).hide();
    ...
    Donation_Form.prototype.scroll_to_top = function() {
        ...
        var $modal = this.form.parents( '.hs-modal' );
    ...
    Donation_Form.prototype.cancel_processing = function( errors ) {
        ...
        this.form.trigger( 'hspg:form:cancelled', this );
    };
    ...
    Donation_Form.prototype.get_required_fields = function() {
        var fields = this.form.find( '.hs-fieldset .required-field' ).not( '#hs-gateway-fields .required-field' ),
    ...
    Donation_Form.prototype.validate = function() {
        ...
        this.form.trigger( 'hspg:form:validate', this );
...
( function( exports, $ ){
    var Toggle = function() {
        ...
        var hide_target = function() {
            get_target( $(this) ).addClass( 'hs-hidden' );
        };
        ...
        /**
         * Toggle event handler for any fields with the [data-hs-toggle] attribute.
         *
         * @access private
         */
        var on_toggle = function() {
            ...
            $target.toggleClass( 'hs-hidden', ( function(){
                    if ( $this.is( ':checkbox' ) ) {
                        return ! $this.is( ':checked' );
                    }
                    return false === $target.hasClass( 'hs-hidden' );
                })()
            );
         ...
         */
        var init = function() {
            $( '[data-hs-toggle]' ).each( hide_target );
        };

        // Initialization only required once.
        $( 'body' ).on( 'click', '[data-hs-toggle]', on_toggle );
 ...
 * @param string compare If compare is left empty, use the Hspg version.
...
( function( $ ) {
    ...
    $( document ).ready( function() {
        ...
        $( '.hs-donation-form' ).each( function() {
