	...
	var Datepicker = function( $el ) {
		...
		var options = {
			...
			beforeShow	: function( input, inst ) {
				$('#ui-datepicker-div').addClass('hs-datepicker-table');
			}
		}
...
( function($){
	...
	var setup_campaign_end_date_field = function() {
		$( '#campaign_end_date' ).on( 'change', function() {
			var $field = $( this ),
				date   = $field.val(),
				$span  = $field.siblings( '.hs-end-time' ),
	...
	var setup_hs_ajax = function() {
		$('[data-hs-action]').on( 'click', function( e ){
			var data 	= $(this).data( 'hs-args' ) || {},
				action 	= 'hs-' + $(this).data( 'hs-action' );
	...
	var setup_hs_toggle = function() {
		$( '[data-hs-toggle]' ).on( 'click', function( e ){
			var toggle_id = $(this).data( 'hs-toggle' ),
				toggle_text = $(this).attr( 'data-hs-toggle-text' );

			if ( toggle_text && toggle_text.length ) {
				$(this).attr( 'data-hs-toggle-text', $(this).text() );
	...
	var setup_advanced_meta_box = function() {
		var $meta_box = $('#hs-campaign-advanced-metabox');

		$meta_box.tabs();

		var min_height = $meta_box.find('.hs-tabs').height();
	...
	var toggle_custom_donations_checkbox = function() {
		var $custom = $('#campaign_allow_custom_donations'),
			$suggestions = $('.hs-campaign-suggested-donations tbody tr:not(.to-copy)'),
	...
	var setup_sortable_suggested_donations = function(){
		$('.hs-campaign-suggested-donations tbody').sortable({
	...
	var add_suggested_amount_row = function( $button ) {
		var $table = $button.closest( '.hs-campaign-suggested-donations' ).find('tbody');
	...
	var reindex_rows = function(){
		$('.hs-campaign-suggested-donations tbody').each(function(){
	...
	var setup_dashboard_widgets = function() {
		var $widget = $( '#hs_dashboard_donations' );

		if ( $widget.length ) {
			$.ajax({
				type: "GET",
				data: {
					action: 'hs_load_dashboard_donations_widget'
				},
				url: ajaxurl,
	...
	var setup_actions_form = function() {
		var $form = $( '.hs-actions-form-wrapper' ),
			$select,
			$submit,
			$button;
		...
		$select = $form.find( '.hs-action-select' );
		$submit = $form.find( '.hs-actions-submit' );
		...
		$select.on( 'change', function() {
			...
			$form.find( '.hs-action-fields' ).each( function() {
				var $el = $( this );
	...
	var setup_donor_select = function() {
		...
		var get_user_fields = function() {
			...
			$( '#hs-user-fields-wrap' ).find( 'select,input,textarea' ).each( function() {
		...
		$( '.hs-admin-donation-form #donor-id' ).on( 'change', function() {
			...
			$user_fields = $( '#hs-user-fields-wrap' );
			$user_fields.addClass( 'loading-data' );

			data = {
				action   : 'hs_get_donor_data',
				donor_id : donor_id,
	...
	$(document).ready( function(){
		...
		if ( CHARITABLE_ADMIN.Datepicker ) {
			$( '.hs-datepicker' ).each( function() {
				CHARITABLE_ADMIN.Datepicker( $(this ) );
			});
		}

		$( '#hs-settings, body.post-type-campaign form#post, body.post-type-donation form#post' ).each( function(){
		...
		$('[data-hs-add-row]').on( 'click', function() {
			var type = $( this ).data( 'hs-add-row' );
		...
		$('.hs-campaign-suggested-donations').on( 'click', '.hs-delete-row', function() { console.log('clicked');
			delete_suggested_amount_row( $(this) );
			return false;
		});

		$('body').on( 'click', '[data-campaign-benefactor-delete]', function() {
			var $block = $( this ).parents( '.hs-benefactor' ),
				data = {
					action 			: 'hs_delete_benefactor',
