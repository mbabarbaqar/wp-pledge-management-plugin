...
( function( $ ) {
    ...
    CHARITABLE.Donation_Form.prototype.validate_card_number = function() {
        ...
        var require_cc = $( '#hs-gateway-fields-' + this.get_payment_method() + ' #hs_field_cc_number' );
    ...
    $body.on( 'hspg:form:initialize', function() {
        $body.on( 'input', 'input[name="cc_number"]', on_input_card_number );
    });

    $body.on( 'hspg:form:validate', function( event, helper ) {
         // Only run validation on the main donation form.
         if ( 'make_donation' === helper.get_input( 'hs_action' ).val() ) {
